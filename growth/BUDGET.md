...
| 2026-09-08 → 09-20 | Balance $0 again, five days after the 09-03 top-up was first drawn on | Same shape — but now every other technique is a `noop` too (`area_pages`, `service_pages` and `money_pages` have exhausted their queues), so a no-credit morning produces **nothing at all**, not merely less. **Thirteen** consecutive mornings of `new: 0, changed: 0` with every step reporting `ok` or `noop` — the longest empty-balance stall this account has had, and it has now cost **more** mornings than the engine has ever worked: thirteen dead against thirteen productive in the whole of 08-12 → 09-20 |
...
pattern that was noticed. Over the forty days 08-12 → 09-20 the engine
did billable work on **13 of 40**; an empty balance accounts for **20** of the
27 lost days and the August crash for the other 7.

The current stall is on its **thirteenth** morning. Productive runs have been 5,
3 and 5 mornings — thirteen productive mornings in total since 08-12, so as of
2026-09-20 **this one stall has cost more mornings than the engine has ever
worked**. The engine is idle more often than it works, and the ratio is getting
worse.

**One correction to how this file has argued, recorded 2026-09-20.** Everything
above treats funding as the top constraint. The pre-registered coverage-versus-
rank test came due on 09-20 and failed: coverage rose 32.3% → 42.7% while the
goal metric `top3` fell 2 → 0. Coverage is what a funded morning buys. So the
auto-reload is still worth doing — it is cents, and a dead engine cannot be
debugged — but it should be argued for as hygiene, not as the lever that moves
the goal. The reasoning is in the 2026-09-20 journal entry.
...
the only lever left during a stall, and as of 2026-09-13 the four zero-cost
techniques were all no-ops — three legitimately, and `internal_links` because
of a bug rather than because the work was done. With that bug fixed
(`_nearby_block`, same date), a deploy gives the engine thirteen page edits it
can make on an empty balance. That is the whole of what an unfunded engine can
still do, and it is not nothing.
