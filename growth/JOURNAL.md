...
> **Editorial note, added 2026-07-27 by a later review-agent run:** the entry
> immediately below this note was committed at 2026-07-27T00:33:38Z (commit
> `75cd2d9`) and was silently deleted six hours later when the engine's own
> `93ff2a2` publish overwrote this file with the droplet's local copy, which
> had never seen it. It is restored here, verbatim, in its original
> chronological position, because a decision log that can lose entries to a
> publish-ordering bug isn't the append-only record it claims to be. See the
> 2026-07-27 — review agent entry near the bottom of this file for the root
> cause, the fix applied to `growth/publish_state.sh`, and why recommendation
> #6 below (add FAQPage schema) was already stale the day it was written.
