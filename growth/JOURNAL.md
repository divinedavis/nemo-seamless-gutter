...
## 2026-08-14 — review agent

### Lead: the Philadelphia "flood" is almost certainly rank-tracker robots, not searches — and the code that would strip it out has been sitting undeployed for ten days

Three corrections and one piece of good news.

**The flood is not a market signal, not Google mis-targeting, and it will not
age out. It looks like automated rank tracking.** I pulled the 40
`discovered_untracked` rows apart today instead of reading their total, and the
structure is the tell:

- **21 distinct towns × 7 head-term templates.** The towns are Abington,
  Audubon, Blue Bell, Devon, Eagleville, East Norriton, Exton, Glenside,
  Hatboro, Horsham, Lower Gwynedd, Malvern, Norristown, Phoenixville, Plymouth
  Meeting, Radnor, Royersford, Spring City, Upper Merion, Wayne, Willow Grove.
  The templates are `seamless gutter contractors|contractor|company|
  installation companies|installation|seamless gutter` + `<town> pa`. That is a
  **matrix**, not a population of humans typing.
- **12,067 impressions and exactly zero clicks.** Not "almost none" — zero,
  across all 40 rows, spanning positions 1 to 35.1. Real human impressions at
  position 10-14 over a 28-day window produce *some* clicks.
- **The implied volume is impossible.** `seamless gutter contractors glenside pa`
  shows 530 impressions = **18.9/day**, every day, in a borough of about 8,000
  people. `seamless gutter installation companies eagleville pa` shows 213.
  Nobody types that phrase 19 times a day.

Current research says this is a known and recently-quantified phenomenon:
Google's removal of the `&num=100` parameter caused industry-wide GSC impression
collapses, and the size of those drops is what exposed how much of the
"impressions up, clicks flat" pattern was rank trackers and SERP scrapers rather
than people ([Practical Ecommerce](https://www.practicalecommerce.com/did-google-just-prevent-rank-tracking),
[Search Engine Land](https://searchengineland.com/why-google-search-console-impressions-dropped-interpret-data-463677),
[Sort the Clicks](https://sorttheclicks.com/insights/impressions-but-no-clicks/)).
The most likely reading here: a Philadelphia-area gutter competitor or its
agency runs a daily rank-tracking job over a keyword × city matrix, NEMO's site
sits in those SERPs at page 2-4, and every scrape logs an impression.

**Three prior conclusions in this journal change as a result, including one of
my own from yesterday:**

1. *"It will age out on 09-01."* Probably wrong. A block ages out; a daily job
   does not. Prediction 3 is revised below.
2. *"The Schuylkill section may have caused it."* Effectively dead. The flood is
   Montgomery/Chester County, the section names Schuylkill County, and a rank
   tracker does not read your prose. Recommendation 5 below survives on core-update
   grounds alone, and I am dropping the causal claim.
3. *"471 impressions at position 1 for `gutter installer` with zero clicks."*
   That was my headline number yesterday for the homepage-title fix. It is
   **16.8 impressions/day on a generic head term with zero clicks** — the same
   bot signature. The title change is still right, but not for that reason, and
   I should not have led with a number I had not tested.

**The good news: credit is back and the engine is not finished.** After two dry
days, `strengthen_pages` shipped a section ("What to Ask Gutter Installers in
York County Before You Hire" → `/services/seamless-gutter-installation.html`) and
`scout` ran clean, adding T060-T062 and 4 keywords. The 08-13 commit
`363e37c` — *"Record that the engine has run out of work, not just credit"* — is
**falsified on its first funded day.** It had work.

### Where the numbers stand

**The goal metric: `top3` = 2 of 170 tracked queries, `share_pct` 1.2%. Flat for
five days; the underlying count of 2 has not moved since 08-10.** Target 50%.
...
**Zero top-3 positions in all seven named towns, day nineteen.** `coverage_pct`
fell 35.4 → **34.1** — not because coverage dropped, but because the scout added
9 queries to the denominator. `matched` stuck at 23 for the seventh day.

**Search Console: 840 rows / 23 matched / 11 clicks / 24,745 impressions / avg
position 24.8.** Row growth **+43** today against yesterday's +147. So
yesterday's "twenty-fold jump, the flood re-accelerated" was **a one-day spike,
and I over-read it.** It is still ~6× the +7/day baseline of early August, but it
is decelerating, not accelerating. Clicks 10 → 11, the first movement in four days
and one click.

Given the section above, **`avg_position` 24.8 and site-wide CTR 0.044% should not
be quoted as facts about this business again.** Roughly half of all impressions in
the window are in the 40 out-of-area rows alone, and the invisible tail below the
40-row cap is likely more of the same.

**Traffic — flat and small.** 7-day median visitors **9** (previous 7: 7); sum 70
vs 72. Pageviews median 14 (13). Organic median **2** (1). `ai_visitors` is **0
for all 20 days on record**. Not zero-and-suspicious — `visitors` runs 3-17/day,
so the measurement is alive; this is a real, small site.

**Leads.** `bookings_all_time` **3**, `phone_leads_all_time` **0**, `call_taps`
**1 all-time** (08-12), `ai_calls` 0 since 07-30. Two bookings landed in the last
seven days (08-10, 08-11) against zero in the prior seven — two events, not a
trend.

**Ledger: 62 techniques — 11 active, 1 retired, 50 candidates.** All 11 active
were auto-activated on 2026-07-27 and every one is an on-site engine task.
**`scoreboard.does_not_work` is `[]` on day nineteen: zero techniques activated,
zero rejected, ever.** The pile grew by 3 today.

### Did previous changes work?

**Prediction 1 — 7-day median `organic_visitors` ≥ 2 by 2026-08-16. RESOLVED
YES, two days early.** The window is 08-10→08-16 and four of its seven days are
banked at ≥2 (08-10: 2, 08-11: 3, 08-12: 2, 08-13: 2). A 7-value median needs
four values ≥2; it has four. The remaining three days cannot change it. Verdict:
**passed** — with the caveat I attached when I made it, that this was a
prediction about whether a number that oscillates 0-3 lands on 2 rather than 1.
It is not evidence that anything worked.

**Prediction 2 — `call_taps` > 0 by 2026-09-01. Resolved yes on 08-13.** Nothing
new: still 1 tap all-time, no second tap on 08-13.

**Prediction 3 — the 08-04 impression block ages out on 2026-09-01 (impressions
down 15,000-18,500). Now expected to FAIL, and for a better reason than
yesterday's.** Yesterday I said the accrual was still accruing. Today's +43 says
the accrual is slowing, but the bot diagnosis says the mechanism is a recurring
job, not a one-time event. **Revised call: on 2026-09-01 impressions stay above
18,000.** If they collapse toward 3,000-5,000 instead, my bot reading is wrong
and it really was a block.

**Prediction 4 (`keywords_added` < 5 after the new scout deploys) and Prediction
5 (`by_intent.price.covered` moves off 3).** **Cannot evaluate, sixth day — not
deployed.** `price` is now **3 of 37** (was 3 of 35): the denominator grew, the
numerator did not. Worst-covered intent in the file; `hire` is 50 of 105.

**Prediction 6 — if credit returns and `strengthen_pages` still finds nothing
within three days, "the engine is finished" is permanent. RESOLVED NO on day
one.** It found work and shipped it. The on-site engine is not out of work; it
was out of money.

**The deploy — recommended 08-10, 08-11, 08-12, 08-13. Not done, and the gap is
worse than I reported.** Yesterday's entry put the droplet at "code from on or
before 2026-08-09", inferred from the missing `code_version`. That was too
generous. `tracked_totals()` landed in `gsc.py` in commit **9479b5c on
2026-08-04** — *"Measure York County separately from whatever else Google shows
the site"* — and `sync()` writes its output into `gsc_last["tracked"]`
(`gsc.py:286-290`), which `snapshot.py:198` spreads into the published block.
Today's `gsc` block has keys `date, ok, connected, rows, matched, clicks,
impressions, avg_position` and **no `tracked`**. `sync()` ran today (the block is
dated 2026-08-14), so the droplet's `gsc.py` predates 2026-08-04. **The deploy
gap is ten days, not five.**

That matters more than it did yesterday, because `tracked_totals` is precisely
the fix for the problem at the top of this entry: it reports clicks, impressions
and average position **over the tracked York County universe only**, discarding
the Philadelphia rows. Its own docstring calls the site-wide reading "a fire" and
the tracked reading "a different market's SERP, ninety miles away". The correct
county number has existed in `main` for ten days and nobody can see it.

**Not actioned, with day counts:** the engine deploy **day 6** (and `gsc.tracked`
specifically **day 10**); York in the homepage title **day 14**; the live
Schuylkill section **day 5**; the Akron paragraph at
`services/gutter-guards.html:163` **day 17**; the 08-12 call-tap question **day
1**; T016 GBP category **day 19**; T007 reviews **day 19**; T033 review replies
**day 3**; T011 LSA decision **day 16**; `?utm_source=gbp` **day 12**; the goal
denominator **day 7**.

### A small reporting caveat, and a correction to this prompt

**Two pages changed on the droplet today; `last_build` reports one.**
`services/gutter-cleaning-repair.html` gained a "Downspout Installation and
Replacement in York, PA" section that appears nowhere in today's build log.
`strengthen_pages` returns immediately after its first successful write
(`techniques.py:1055-1056`), so it cannot have written both. The benign reading —
which I believe — is that credit was topped up during 08-13 and someone re-ran
the build off-schedule; `last_build` holds only the most recent run, so an
off-schedule run is invisible. Not a bug to chase, but `changed: 1` undercounts
and `last_build` should not be read as "everything that happened today".

**This prompt's "Known blockers" section is 17 days stale and should be
corrected.** It describes the 2026-07-28 state and says to expect the API usage
cap "until 2026-08-01". The actual failure mode since then has been an **empty
credit balance**, dry on 08-05, 08-12 and 08-13, and today it is funded and
running. The section also says Search Console showed "77 rows, 17 matched, 3
clicks, 429 impressions" and "the county bucket was 2 of 50" — today it is 840
rows, 23 matched, 11 clicks, 24,745 impressions, county 2 of 92. Anyone reading
the prompt as current will badly misjudge the scale of the flood.

### What I researched today

- **Rank-tracker and bot inflation of GSC impressions** — the `&num=100` removal
  and the impression collapses that followed
  ([Practical Ecommerce](https://www.practicalecommerce.com/did-google-just-prevent-rank-tracking),
  [Search Engine Land](https://searchengineland.com/why-google-search-console-impressions-dropped-interpret-data-463677),
  [Sort the Clicks](https://sorttheclicks.com/insights/impressions-but-no-clicks/)).
  This is the basis of the lead section and the one genuinely new idea today.
- **The GSC logging bug, checked and rejected as an explanation.** A
  platform-wide bug inflated impressions from 2025-05-13, acknowledged
  2026-04-03, fixed forward 2026-04-27, with history not rewritten
  ([Passionfruit](https://www.getpassionfruit.com/research/your-search-console-data-has-been-wrong-for-a-year),
  [seo-kreativ](https://www.seo-kreativ.de/en/blog/gsc-impressions-bug-google-logging-error/)).
  **Rejected on dates:** this site's flood began 2026-08-04, over three months
  after the fix shipped. Worth recording because it is the obvious wrong answer.
- **GBP photos as a 2026 ranking pillar** — Vision AI scanning photos to verify
  category, services and location; consistent weekly uploads weighted above bulk
  dumps; a 250+ photo benchmark cited for materially better performance
  ([Henry David Photography](https://www.henrydavidphotography.com/resources/blog/google-business-profile-photography-local-seo-2026),
  [King of Pressure Wash](https://www.kingofpressurewash.com/blog/google-my-business-profile-optimizing-your-images-in-2026),
  [ClickRank](https://www.clickrank.ai/google-local-pack/)).
  **Filed as a refinement, not a new technique** — T022 (itemised GBP Services +
  weekly job photo) and T043 (30-second phone video proof) already cover this,
  and photos were recommended on 08-01. What is new is the *cadence and volume*
  guidance, which sharpens T022.
- **LSA cost-per-lead benchmarks** — $53 blended across 888 contractors and
  126,650 leads (Feb 2026), roofing an outlier at $162, and **gutters is a
  service option under the Roofing category rather than its own category**
  ([SearchLight](https://searchlightdigital.io/google-local-service-ads-cost-per-lead/),
  [Booked Friday](https://bookedfriday.com/local-services-ads-cost),
  [PushLeads](https://pushleads.com/google-local-service-ads-in-2026-the-complete-setup-and-optimization-guide-for-contractors/)).
  **Filed as a refinement to T011**, which has been undecided for 16 days. The
  practical detail Eric needs: signing up means entering under Roofing, where
  lead prices are set by roofers with much larger job values.
- **Rejected, with reasons.** (1) *Adding the Montgomery/Chester towns to the
  site* — seventh rejection; doorway pages, and now doubly pointless since the
  "demand" is robots. (2) *Chasing the GSC Generative AI report* — still UI-only,
  not in the API; rejected 08-05 and 08-13. (3) *Filing today's photo and LSA
  research as new techniques* — **I filed nothing.** The pile is 50 with zero
  ever picked; adding to it is the opposite of help. (4) *GBP posting cadence*,
  (5) *padding the GBP service area* — both re-rejected.

### Recommendations

**Nothing in this commit is live.** The site runs from
`/var/www/nemo-seamless-gutter`, which is not a git checkout, and
`publish_state.sh` copies droplet → repo only. Editing HTML here would be
overwritten by tomorrow's publish. Every item needs a human on the droplet or in
a dashboard.

1. **Eric: twenty minutes on the Business Profile — primary category first, then
   count the service-area list.** *(Free. T016/T051, day 19.)* Primary category
   remains the highest-weighted single local-pack factor in the 2026 reports, with
   a documented position-1→31 failure from one word changed. Screenshot before
   touching anything, change at most one field, and **count the service areas** —
   padding is a documented negative and this business has already demonstrated a
   taste for claiming places it does not serve. *Why it is #1:* nineteen days, zero
   calls, zero top-3 in any named town, and leaf-fall opens in about six weeks.
   Everything below this is measurement or maintenance; this is the only item that
   plausibly moves the map pack before the season. *How you would know:* `york`
   bucket `covered`/`top3` moves off 5/0, or `local_visitors` lifts off ~0, within
   a month. *Checked:* T016 and T051 are both `status: candidate, activated: null`
   in `snapshot.json`; no site-side change is involved, so no deploy.
2. **Divine: deploy the engine — and know that `gsc.tracked` alone justifies
   it.** *(Minutes. Day 6; day 10 for the piece that matters.)* Ship `gsc.py`,
   `snapshot.py`, `metrics.py`, `techniques.py`, `keywords.py`, and `scout.py` +
   `growth_daily.py` **as a pair** or the scout raises `TypeError`. *Why it moved
   up:* it is the only way to see the York County numbers with the robot
   impressions removed. Right now every rank figure in this journal is an average
   over a dataset that is roughly half machine traffic. *How you would know:*
   tomorrow's snapshot carries `gsc.tracked`, `gsc.pages`, `keywords.ranked`,
   `code_version`, `crawl_*`, and more than 40 `discovered_untracked` rows.
   *Checked:* all absent from today's snapshot, against `snapshot.py:178-221`;
   `gsc.py:286-290` writes `tracked` into `gsc_last`; `len(discovered_untracked)`
   is exactly 40; `tracked_totals` traced to commit 9479b5c, 2026-08-04.
3. **Eric: answer every existing Google review, in one sitting.** *(Free, no
   customer contact. T033, day 3.)* Owner response rate is weighted above raw
   review count in the 2026 breakdowns, and replying carries **zero** policy
   surface while Google is actively probing businesses about review incentives —
   which is exactly why this comes before T007 (asking for new reviews). Reply in
   Eric's own words; name the service and town where the review already does.
   *Checked:* T033 is `candidate`/`activated: null`; no site change, no deploy.
4. **Divine: put York in the homepage title.** *(One line, five minutes, free.
   Day 14.)* `/var/www/nemo-seamless-gutter/index.html:16` is
   `<title>Gutter Installer &amp; Contractor | NEMO Seamless Gutter</title>`.
   Make it e.g. `Seamless Gutters in York, PA | NEMO Seamless Gutter` (49 chars,
   inside the 65-char limit at `techniques.py:1281`). **Demoted from #1 with the
   evidence corrected:** the 471 impressions I cited yesterday are almost
   certainly bot impressions, so the gain is not "stop wasting a national
   ranking" — it is simply that a York County contractor's most important page
   should say York, and the engine's own `GEO_ANCHOR` guard
   (`techniques.py:1295-1299`) refuses to write placeless snippets. *Checked:*
   `index.html:16` read today; `improve_ctr` returned `noop` again today.
5. **Divine: delete the two off-area passages from the live site.** *(Five
   minutes, free. Day 5 and day 17.)*
   `services/seamless-gutter-installation.html:227-230` and the Schuylkill FAQ at
   `235-236`; and `services/gutter-guards.html:163`, "homes in Akron, PA and the
   surrounding Lancaster and York County area" → "homes across York County,
   Pennsylvania". **The flood justification is withdrawn** — see the lead
   section. What remains is real on its own: the March 2026 core update targeted
   templated location pages, another core update is expected inside six weeks,
   this site has 15 generated area pages, and prose claiming counties the business
   barely serves is the exact profile. *Checked:* both passages read out of the
   working tree today at the line numbers given; the guards that would prevent
   recurrence (`_off_area_prose`, `techniques.py:1413`; `SERVICE_AREA_WORDS`,
   `techniques.py:1328`) exist in git but are **not deployed**.
6. **Eric or Divine: answer the call-tap question.** *(Five minutes, free. Day
   1.)* On 08-12 one person tapped the call button and no call was recorded. Did
   the phone ring? If yes, `calls.py` is blind to calls landing on Eric's cell and
   `phone_leads_all_time: 0` has been an artifact all along. If no, the site has a
   measured drop-off point for the first time. *Checked:* `metrics.py:493-496`
   de-duplicates on `ip|ua`, so this is one person, not a double-fire; T049 covers
   this and has never been activated.
7. **Someone must decide what the goal metric counts — and the urgency just went
   up.** *(Day 7.)* `top3` has been 1-2 since 08-01 while `tracked_queries` went
   108 → 170, so `share_pct` fell 1.9% → 1.2% **without a single position
   moving**. The scout adds ~5 queries a day, and **now that credit is restored it
   will do that every day**. Left alone, by leaf-fall the denominator is ~400 and
   the headline number reads ~0.5% even if top-3 positions double. Refunding the
   engine made this worse, not better. Either cap the tracked universe, or promote
   `top3 / ranked_known` (2 of 23) to the headline. I have not made this change
   myself: it redefines the number Eric is judged against, which is not mine to
   redefine.
8. **Eric: pick three candidates and reject the rest.** *(Day 7. Pile now 50, up
   3 today; `does_not_work` still `[]` after nineteen days.)* The scout is
   generating options faster than anyone consumes them, and it now has funding to
   keep doing so. A rejection is as valuable as an activation and costs nothing.
...
I considered adding a `device` dimension to the GSC summary — desktop-only
impressions on the Philadelphia rows would turn today's inference into proof,
and `_query()` (`gsc.py:130`) already takes arbitrary dimensions, so it is about
fifteen lines. **I did not write it.** There are already ten days of undeployed
engine code in `main`, including the one function that would fix the actual
problem, and an eleventh unshipped commit is not help — it is inventory. More to
the point, the device split would not change any action: the recommendation is
the same whether the flood is 95% or 100% robots. Recorded here so it can ride
along with recommendation 2.

### Reasoning and uncertainties

Day nineteen. Two top-3 queries county-wide, zero in any named town, zero AI
visitors across the entire twenty-day series, zero phone leads ever, one call
tap, three bookings.

The shape of the problem changed today, though. For three weeks this journal has
treated the Search Console panel as a noisy but real picture of the market. If
today's reading is right, roughly half of it is a machine in Montgomery County
and the honest county-level dataset is small: 23 matched queries, 11 clicks. That
is not worse news — it is the same business it always was — but it means several
paragraphs of prior analysis were interpreting robot output, and the correction
belongs on the record.

**Where I am least confident. Four things.**

*First, the bot diagnosis itself.* The evidence is circumstantial but it stacks:
a 21×7 town-by-template matrix, exactly zero clicks on 12,067 impressions, and
per-query daily volumes that exceed any plausible search demand in boroughs of a
few thousand people. What I do **not** have is the device split, which is the
clean test. I would put this at 85%. If it is wrong, the alternative is that
Google genuinely believes this site is a Philadelphia-suburb gutter contractor,
which would be a much larger problem and would need a different response.

*Second, whether recommendation 1 is really above recommendation 2.* Correct
measurement is worth a lot and I have been asking for the deploy for six days.
But measurement does not ring a phone, and this business has six weeks until its
season. If Divine has five minutes and Eric has twenty, both should happen; if
only one does, it should be Eric's twenty.

*Third, I have now written four consecutive entries whose top recommendations
were not acted on.* Nineteen days, zero techniques activated, zero rejected. At
some point the correct conclusion is not "the recommendations need to be
sharper" but "this report is not reaching anyone who can act, or it is too long
to act on". If the same list is still unactioned on 08-21, I think this review
should drop to weekly and shrink to three lines, and the daily slot should become
a one-line reminder of the single highest-value open item.

*Fourth, unchanged and still unresolved:* I cannot judge the **quality** of the
generated copy from here. I verified the count — 32 pages carry `FAQPage` schema
— but counting schema blocks is not reading them, and a rewriter judging its own
output is not evidence. If someone reads five guide pages and finds them thin,
"the on-site work is saturated" collapses and there is plenty of work left.

**What would change my mind, dated.** (1) **2026-09-01:** if impressions fall
below 5,000, the flood was a block and my bot reading is wrong. If they hold
above 18,000, it is a recurring job and every "it will age out" sentence in this
journal — including four of mine — is wrong. (2) If the deploy lands and
`gsc.pages` shows the homepage as a `ctr_candidate`, the improve_ctr blocking
diagnosis from 08-13 is wrong. (3) If the deploy lands and `gsc.tracked` shows
county impressions in the thousands rather than the low hundreds, the flood is a
smaller share of the dataset than I claim here. (4) If recommendation 4 ships and
`gutter installer` impressions do **not** fall, that confirms bots rather than
...
## 2026-08-16 — review agent

### Lead: the first full beacon day landed at 2, and the beacon is not the problem — the number is real

Yesterday I set a dated test: *"2026-08-16: the first full beacon day lands. If it
reads 6-10, the log-era numbers were roughly 2× inflated; if it reads 0-2, this
site has almost no human traffic and every SEO recommendation here is premature
next to the map pack."*

**It reads 2.** Visitors 2, pageviews 2, organic 1, direct 1.

Before treating that as the collapse it looks like, the comparison has to be
same-weekday, because 08-15 is a Saturday and Saturdays were already the floor:

| | Sat 08-01 (log) | Sat 08-08 (log) | **Sat 08-15 (beacon)** |
|---|---|---|---|
| visitors | 5 | 3 | **2** |
| organic | 0 | 0 | **1** |
| direct | 5 | 3 | **1** |

And the partial Friday, against its own weekday: organic was 1 (07-31) and 0
(08-07) in the log era; the beacon read **2** on 08-14 while covering only ~45%
of the day.

**So organic went up across the instrument change, on both beacon days, against
the matching weekday.** That is the finding, and it settles a question I could
not answer yesterday. A beacon that was silently dropping real people would drop
them from every bucket. Instead the bucket that survived is organic and the
bucket that vanished is direct — 5 and 3 on the comparable Saturdays, 1 now.
That is precisely the failure mode `588ae32` was written to fix: the log-era
`direct` bucket was mostly clients that never execute JavaScript.

**Conclusion: the beacon is working, and this site's real human traffic is
roughly 2-4 people a day, nearly all of it organic search.** The honest
version of the drop is 3-5 → 2 on comparable days, not 9 → 2. That is a smaller
correction than yesterday's table implied and a worse absolute number than any
figure in this journal before 08-14.

I still cannot *prove* it, because rec 1 has not shipped: today's snapshot has no
`traffic.log_visitors`. The weekday argument above is the best I can do without
it and it is inference, not verification.

**One thing to watch, not yet a finding.** Pageviews equal visitors exactly on
both beacon days — 2/2 and 2/2, a pages-per-visit of 1.00. `analytics.js`
fires the pageview IIFE on every page load (`analytics.js:50-70`), so a
two-page session should log two hits. Two days of n=2 proves nothing. If
pages-per-visit is still 1.00 after a full week, either every visitor bounces or
the beacon fires once per session, and those need different fixes.

**Do not quote a beacon median yet.** There are two beacon days, one of them
partial. The first honest 7-day beacon window closes **2026-08-21**.

### Where the numbers stand

**The goal metric: `top3` = 2 of 182 tracked queries, `share_pct` 1.1%. The
count of 2 has not moved since 08-10 — seven days.** Target 50%.
...
**Zero top-3 positions in all seven named towns, day twenty-one.** `top10` 7 and
`ranked_known` 23, both unmoved for over a week. `coverage_pct` held at 33.5
(61/182 against yesterday's 59/176) — covered +2, tracked +6.

**Search Console: 853 rows / 23 matched / 14 clicks / 24,971 impressions / avg
position 24.8.** Row growth +7 (from +147, +43, +6). `matched` stuck at 23 for
the **ninth** day. `avg_position` flat at 24.8 for the fourth. Clicks +2.

**The flood is confirmed dead, on three days of evidence rather than one.** I
diffed today's `discovered_untracked` against yesterday's published snapshot:
**0 added, 0 removed, 0 changed** — the same 40 rows, the same 12,067
impressions, the same 0 clicks, `gutter installer` still exactly 471 at position
1. That is now three consecutive byte-identical days. The 28-day window rolled
forward twice and neither the day that entered nor the day that left touched a
single one of those rows, which is only possible if the burst is entirely
interior to the window and finished. **Reading stands: an event that ran roughly
08-04 → 08-11, not a recurring job.** Yesterday's revision was right and
Thursday's was wrong.

**Traffic.** Covered above. `ai_visitors` is **0 for all 22 days on record**.

**Leads.** `bookings_all_time` **3**, `phone_leads_all_time` **0**, `call_taps`
**1 all-time** (08-12, no second tap in four days), `ai_calls` 0 since 07-30.
Nothing new.

**Ledger: 68 techniques — 11 active, 1 retired, 56 candidates.** All 11 active
auto-activated 2026-07-27; every one is an on-site engine task.
**`scoreboard.does_not_work` is `[]` on day twenty-one: zero techniques activated
by a human, zero rejected, ever.** The pile grew by 3 again today.

**The engine is not blocked, and the prompt that briefs me is stale on this
point.** My standing instructions still say to expect the scout to fail on a
self-imposed API spend cap "until 2026-08-01" — that is sixteen days in the past
and today's `last_scout` is `ok: true` with three techniques and six keywords
added, on top of a clean build that shipped two pages. **Someone should delete
that paragraph from the prompt**; it invites a lead paragraph about a blocker
that does not exist. Per Step 3b, the data wins and I am recording it here rather
than reasoning from it.

### Did previous changes work?

**Prediction 1 — 7-day median `organic_visitors` ≥ 2 by 2026-08-16. PASSED, and
now closed.** The window 08-10→08-16 holds 2, 3, 2, 2, 2, 1 with 08-16 unseen;
the 4th of seven sorted values is 2 whatever the last one is. Stated caveat
stands and is worth repeating because it is the whole lesson of this prediction:
**this was a prediction about whether a number that oscillates 0-3 lands on 2
rather than 1. It is not evidence that anything worked.** I will not make another
of that shape.

**Prediction 2 — `call_taps` > 0 by 2026-09-01. Resolved yes on 08-13.** Still 1
tap all-time. Four days, no second.

**Prediction 3 — the impression flood.** Corrected test from yesterday stands
unchanged and is not yet testable: impressions should fall sharply between
**09-04 and 09-11**, and sit near 6,000-9,000 by **09-13**. Today's third
identical day of the 40 rows is consistent with it. Above 18,000 on 09-13 and the
event reading is wrong.

**Predictions 4 and 5 — `keywords_added` < 5 after the scout deploys, and
`by_intent.price.covered` moving off 3. Eighth day, still not deployed, so 4 is
not evaluable.** Today `keywords_added` = 6.

**Prediction 5 is evaluable, and it has failed for a reason I can now name.**
`price` is **3 of 42** — 3 of 35, then 3 of 37, then 3 of 38, now 3 of 42. The
numerator has not moved in three weeks. The mechanism is in `techniques.py:966`
and it is not bad luck; see recommendation 4.

**Yesterday's `snapshot.py` change (commit `9e18cbf`): committed, not deployed.**
Today's `traffic` block has no `log_visitors`/`log_pageviews`, which is exactly
the check it adds.

**The deploy — recommended 08-10 through 08-15, now day 9 for `snapshot.py`.**
Unchanged from yesterday and independently re-verified against today's file:

| what | in `main` since | visible in today's snapshot? |
|---|---|---|
| `gsc.tracked` (`9479b5c`) | 08-04, **12 days** | no |
| `crawl_*` series (`727deec`) | 08-07 | no |
| `code_version` (`870a3dc`) | 08-09 | no |
| `keywords.ranked` (`snapshot.py:200`) | before 08-09 | no |
| `log_visitors` (`9e18cbf`) | 08-15 | no |

**I checked before recommending anything new here, and it changed what I
recommend.** I was going to propose publishing a per-town `top10` so I could see
which town is closest to breaking through. `snapshot.py:200` already publishes
`keywords.ranked` — every ranked query with its town and position — from which
per-town top-10 is a two-line computation. **The thing I wanted already exists in
`main` and is undeployed.** Same for the beacon discrepancy check. There is no
snapshot work left to do; there is a deploy.

**Not actioned, with day counts:** `snapshot.py`/`gsc.py` deploy **day 8**; PA
HIC registration **day 2**; York in the homepage title **day 16**; the live
Schuylkill passages **day 7**; the Akron paragraph **day 19**; T016 GBP category
**day 21**; T007 reviews **day 21**; T033 review replies **day 5**; T011 LSA
decision **day 18**; `?utm_source=gbp` **day 14**; the goal denominator **day
9**. Re-verified in the working tree today: `index.html:16` is still
`<title>Gutter Installer &amp; Contractor | NEMO Seamless Gutter</title>`;
`services/seamless-gutter-installation.html:227-236` still names Pottsville,
Frackville and Mahanoy City; `services/gutter-guards.html:163` still says
"homes in Akron, PA"; and a grep of every `*.html` for
`HIC #|home improvement contractor|PA #|licensed and insured|fully insured`
returns **zero hits**.

**A structural fact I should have stated five days ago: I cannot fix any of
those from this repo, and neither could any future version of me.**
`publish_state.sh` does `copy index.html` and
`rsync -a "$DOCROOT/$dir/" "$CLONE/$dir/"` for `areas`, `guides` and `services`,
then `git add -A` and commits. All four are **droplet → repo**. If I edited
`index.html` or the Akron paragraph here, tomorrow's 06:00 publish would
overwrite my edit with the docroot's version and commit the reversion. So
"delete the Akron paragraph" is not a task I have been declining to do — it is
physically outside this repo's reach, and it needs a hand on the droplet.
`growth/*.py` is the exception: `publish_state.sh` copies only
`growth/snapshot.json` and `growth/JOURNAL.md`, so engine code committed here
survives and is what a deploy would ship.

### What I researched today

- **The Google Business Profile Q&A section is gone, and it takes half of a
  ledger technique with it. NEW.** Google shut the My Business Q&A API on
  2025-11-03, began deprecating the public Q&A section on 2025-12-03, and
  replaced it from ~March 2026 with Gemini-powered **"Ask Maps"**, which
  generates answers on the fly from the profile's **categories, attributes,
  services, hours, photos and review text**
  ([EmbedSocial](https://embedsocial.com/blog/google-my-business-features/),
  [PinMeTo](https://www.pinmeto.com/blog/google-business-profile-updates-2026/),
  ...
  Five independent sources; I could not fetch two of them directly (egress
  proxy blocked `pinmeto.com`, `northcountydigital.com` and
  `fogdigitalmarketing.com`), so this rests on search summaries plus corroboration
  across domains rather than a primary Google announcement — treat the exact
  dates as approximate and the removal itself as solid.
  **Consequence for the ledger: T016 is named "GBP primary/secondary category +
  Owner Q&A audit" and its Q&A half no longer exists as a place to type
  anything.** The category half is untouched and still the highest-weighted
  single signal. **The work did not disappear, it moved**: the fields Ask Maps
  reads are exactly T022's itemised Services, T042's hours and T051's service
  areas. Those three stopped being "profile hygiene" and became the source text
  Gemini answers York County gutter questions from.
- **GBP signals are now the largest single block of local-pack weight — 32%,
  ahead of on-page 19%, reviews 16%, links 15% — and primary category is the
  biggest signal inside it.** Proximity still matters but is less dominant for
  service-area businesses spanning multiple towns
  ([SEOLocale](https://seolocale.com/google-map-pack-ranking-in-2026-how-the-local-3-pack-really-works/),
  [Birdeye](https://birdeye.com/blog/google-local-pack/),
  [Predicta Digital](https://predictadigital.com.au/blog/how-to-rank-in-the-google-map-pack-in-2026-whats-actually-changed/)).
  Consistent with what this journal already recorded on 08-15; **no new filing**,
  but it is the third independent week of the same answer to "what would move
  York from 0 top-3", and that answer is not on-site.
- **Perplexity has no Business Profile equivalent — it assembles local answers
  from Bing Places plus the open web, and ChatGPT takes 46.3% of its citations on
  "best X near me" queries from directories**
  ([Seenos](https://seenos.ai/perplexity-seo/perplexity-seo-local),
  [Cheers](https://www.cheers.tech/geo-academy/ai-search-engine-source-differences),
  [Scope](https://scope.online/learn/perplexity-local-business-guide)).
  **This does not justify a new technique — it re-ranks existing ones.** The
  ledger has seven AI-visibility candidates (T025 Brave, T029 Bing Places +
  Apple, T046 bot-fetch check, T053 BuildZoom, T056 third-party lists, T065
  audit) and no stated order. This names a mechanism: **T029's Bing Places half
  is the one with a direct causal path to Perplexity**, and T046 gates all of
  them. Filed as ordering evidence, not as a candidate.
- **Conversion research, filed as support for recommendation 2, not as a
  technique.** Trust badges placed near contact buttons lift conversion ~12.6%;
  responding within 60 seconds lifts it up to 391%; roofing-adjacent trades run
  4-8% site-to-lead
  ([Bullseye](https://www.bullseyemarketingconsultants.com/marketing/high-converting-contractor-websites-the-2026-lead-generation-checklist/),
  [EstateHub](https://www.estatehub.io/articles/2026-benchmarks-lead-conversion-rates-home-services),
  [PipelineOn](https://pipelineon.com/blog/contractor-website-conversion-rate/)).
  **I am deliberately not turning this into a recommendation.** At 2-4 visitors a
  day, a 12.6% conversion lift is 0.3 extra leads per year. Every
  conversion-optimisation idea in this ledger — T023, T024, T054 — is arguing
  about the denominator 2. They are not wrong, they are just worth almost nothing
  until something upstream changes, and the research above says the upstream
  thing is the Business Profile.
- **Rejected.** (1) *FAQPage structured data*, which the AEO sources push hard —
  `techniques.py` already emits FAQPage JSON-LD on every page it generates, and
  32 pages carry it. This is the exact recommendation my standing instructions
  cite as the one that wasted a slot; it is still already shipped. (2) *GBP
  messaging/chat* — Google removed the feature; already flagged in the scout's
  own notes. (3) *Filing today's research as new techniques* — **I filed
  nothing.** 56 candidates, zero ever picked by a human. (4) *A new
  keyword-universe cap proposal* — that is recommendation 6, already open since
  08-08, and re-proposing it as new would be padding.

### Recommendations

**Nothing in this commit is live.** The site runs from
`/var/www/nemo-seamless-gutter`, which is not a git checkout, and
`publish_state.sh` copies droplet → repo only. My `techniques.py` change below
needs a deploy before it does anything.

1. **Divine: deploy `growth/`.** *(Minutes. Day 9.)* Unchanged and now blocking
   five things at once: `gsc.tracked` (12 days), `crawl_*`, `code_version`,
   `keywords.ranked`, and the `log_visitors` beacon check — plus today's
   `techniques.py` change. Ship `scout.py` + `growth_daily.py` **as a pair** or
   the scout raises `TypeError`. *How you would know:* tomorrow's snapshot
   carries `traffic.log_visitors`, `gsc.tracked`, `keywords.ranked` and
   `code_version`. *Checked:* all five absent from today's `snapshot.json`;
   `keywords.ranked` traced to `snapshot.py:200`, `log_visitors` to
   `snapshot.py:47`. **The lead section of this entry is inference that this
   deploy would turn into verification.**
2. **Eric: confirm the PA HIC registration number and put it on the site, the
   Business Profile and the estimate form.** *(Free if already registered. Day
   2.)* Unchanged from yesterday. PA law requires the registration number on all
   advertising distributed in Pennsylvania and a website is advertising; it is
   the only item on this list that is a legal exposure rather than a growth idea.
   Today's research adds a second reason: trust badges beside contact buttons are
   a documented ~12.6% conversion lift, and this site currently carries no
   "licensed", "insured" or registration line anywhere. *If Eric is not
   registered and does ≥$5,000/year, that is the finding and it outranks
   everything else here.* *Checked:* grepped every `*.html` today for
   `HIC #|home improvement contractor|PA #|licensed and insured|fully insured` —
   zero hits; `index.html` footer carries address, phone and a privacy link only.
3. **Eric: twenty minutes on the Business Profile — primary category, then the
   service-area list, then the itemised Services.** *(Free. T016/T051/T022, day
   21.)* This is the sixth consecutive day at or near the top and today's
   research strengthens it twice: GBP signals are 32% of local-pack weight with
   primary category the largest single one, **and** the Ask Maps change means
   those same fields are now the text Gemini answers York County gutter questions
   from. Screenshot first, change at most one field, count the service areas —
   padding is a documented negative. **Ledger correction: skip the "Owner Q&A"
   half of T016. That feature no longer exists.** *Why it stays here:* twenty-one
   days, zero calls, zero top-3 in any named town, ~2-4 human visitors a day, and
   leaf-fall in about four weeks. *How you would know:* the `york` bucket moves
   off 5/0, or `local_visitors` lifts off ~0, within a month. *Checked:* T016,
   T051 and T022 are all `status: candidate, activated: null` in today's
   snapshot.
4. **Divine: deploy the `strengthen_pages` queue change I made today (see
   below) — it is why `price` coverage has been frozen at 3 for three weeks.**
   *(Included in rec 1's deploy; no extra work.)* `techniques.py:966` sorted the
   uncovered queue strictly by intent — `hire`, then `price`, then `check`, then
   `diy` — and this technique ships about one section a day. Today that is **57
   uncovered `hire` queries in front of 39 uncovered `price` ones**, so the first
   price section would have landed around **mid-October, after the leaf-fall
   season the pricing content exists to sell into**. Worse, the queue is re-sorted
   every morning and the scout adds up to 12 queries a day
   (`scout.py:40`), so a refilling `hire` bucket can defer `price` indefinitely —
   which is exactly the observed behaviour: `price` covered 3 while its total went
   35 → 37 → 38 → 42. *How you would know:* `by_intent.price.covered` moves off 3
   within a week of deploy. *Checked:* the ordering is `techniques.py:966`, not a
   ledger technique; `money_pages` shares the same `order` dict
   (`techniques.py:455`) but **I am not touching it** — it returned "no query
   needs its own page" on eleven of the last twelve days, so its ordering
   almost never selects anything and changing it would be noise. That was my
   first draft of this recommendation and the build log refuted it.
5. **Divine: the three live-site edits — homepage title, Schuylkill passages,
   Akron paragraph.** *(Ten minutes total, free. Days 16, 7 and 19.)*
   `index.html:16` → `Seamless Gutters in York, PA | NEMO Seamless Gutter` (49
   chars, inside the 65-char limit at `techniques.py:1281`);
   `services/seamless-gutter-installation.html:227-236` and
   `services/gutter-guards.html:163` → "York County, Pennsylvania". **Grouped and
   marked Divine because, as established above, these cannot be done from this
   repo — a commit here is reverted by the next publish run.** The guards that
   prevent recurrence (`_off_area_prose` `techniques.py:1413`,
   `SERVICE_AREA_WORDS` `techniques.py:1328`) exist in git and ship with the same
   pending deploy.
6. **Someone must decide what the goal metric counts. Day 9, worse again
   today.** `top3` has been 1-2 since 08-01 while `tracked_queries` went 108 →
   **182**. Today the engine covered 2 queries and the scout added 6 — a 3:1 loss
   rate, and `coverage_pct` only held flat by rounding. The denominator grows
   faster than the engine can cover it, so **both headline numbers now fall on a
   good day**. Either cap the tracked universe or promote `top3 / ranked_known`
   (2 of 23) to the headline. **I have not made this change myself:** it
   redefines the number Eric is judged against.
7. **Eric: answer every existing Google review, in one sitting.** *(Free, no
   customer contact. T033, day 5.)* Owner response rate is weighted above raw
   review count and replying carries zero policy surface under the April 2026
   enforcement wave. Ask Maps adds a reason: it reads review text, so replies that
   name the service and the town become answer material. *Checked:* T033 is
   `candidate`/`activated: null`.
8. **Eric: pick three candidates and reject the rest.** *(Day 9. Pile now 56, up
   3 today; `does_not_work` still `[]` after twenty-one days.)*

**On cadence.** Yesterday I said that if the recommendation list is still
untouched on **2026-08-21** this review should drop to weekly and shrink to three
lines, with rec 2 exempted as a legal item. **I stand by that.** Five days.

### What I changed in this repo today

**One code change, in `growth/techniques.py`:** `strengthen_pages` now
round-robins the uncovered queue across intents instead of sorting strictly by
intent, plus the `itertools` import it needs. `hire` still leads every rotation —
it is still the intent that converts — but `price`, `check` and `diy` are reached
on the following days rather than after the hire bucket empties. Rationale and
arithmetic in recommendation 4.

`pytest` is still not installed in this container, so I could not run
`growth/test_techniques.py` — **someone should, before deploying.**
`python3 -m py_compile growth/techniques.py` passes, and I replicated the new
queue construction against today's real intent counts (57/39/18/7) and asserted:
no query lost or duplicated (121 in, 121 out), within-intent order preserved,
first `price` query moves from position ~58 to position **2**, 8 of the first 30
slots go to `price`, and empty / single-item / unknown-intent pools do not raise.

Nothing else. No page copy, no keyword lists — and as established above, page
copy committed here would be reverted by tomorrow's publish anyway. I did not
touch `techniques.json`, `keywords.json`, `results.jsonl` or `state.json`, and
did not activate, retire or re-status any technique.

### Reasoning and uncertainties

Day twenty-one. Two top-3 queries county-wide, zero in any named town, zero AI
visitors across twenty-two days, zero phone leads ever, one call tap, three
bookings. None of that moved today. What moved is that I now believe the traffic
number, and the traffic number is 2.

That belief is the day's real output, and it reorders everything. For twenty days
this journal has argued about on-site work against a visitor count that was
mostly scanners. Against a true 2-4 humans a day, the on-site queue is not
*wrong* — the pages have to exist before anyone can land on them — but no on-site
change can produce a call this month, because there is nobody there to convert.
The three cheapest items on the list (GBP category, service areas, itemised
Services) are the only ones that touch the surface where a York County homeowner
with a falling gutter actually starts, and they are twenty-one days old and free.

**Where I am least confident. Four things.**

*First, the beacon conclusion is inference.* The same-weekday organic comparison
is the strongest argument available to me and it rests on n=2 beacon days against
n=2 log-era Saturdays. If `log_visitors` ships and reads 12 against a beacon 2 on
the same day, my reading survives; if it reads 3 against a beacon 2, then the old
log filter was roughly right all along, the direct bucket was real people, and
the commit's premise — and this entry's lead — were both wrong.

*Second, the intent-ordering fix could be solving a smaller problem than I
think.* I can see 57 uncovered hire queries in the snapshot, but
`strengthen_pages` also skips any query with no host page and any already in the
`strengthened` state list, and **I cannot see either from here** — the real hire
backlog could be 12 rather than 57, in which case price was three weeks away
rather than two months and the change is a minor improvement instead of an
unblocking. The failed `money_pages` version of this same idea is the warning:
its queue looked deep from the snapshot and was empty on eleven of twelve days.
What makes me keep the change anyway is the observed outcome rather than the
queue depth — `price` covered has been 3 for three weeks across four different
denominators, and that is the number a working queue would have moved.

*Third, my research today leaned on search-result summaries for the Ask Maps
finding* because the proxy blocked three of the five sources. Five domains agree
and the direction is unambiguous, but I have not read a Google announcement, and
I would rather flag that than let a ledger correction rest on secondhand text
without saying so.

*Fourth, unchanged and now three weeks old: I cannot judge the quality of the
generated copy from here.* If someone reads five guide pages and finds them thin,
"on-site work is saturated" collapses and there is a great deal left to do.

**What would change my mind, dated.** (1) **2026-08-21:** the first honest 7-day
beacon window closes. If it medians 2-3, the traffic reading above is confirmed
and the map pack is the only game; if it medians 6+, 08-14 and 08-15 were a
weekend trough and I over-read two days again. (2) **Corrected flood test:**
impressions fall sharply **09-04 → 09-11**, near 6,000-9,000 by **09-13**; above
18,000 on 09-13 and the event reading is wrong. (3) **On deploy:** if
`log_visitors` for a beacon-era day is close to `visitors`, the old filter was
fine and the lead of this entry is wrong. (4) **On deploy:** if
`by_intent.price.covered` has not moved off 3 within a week of the queue change
shipping, my diagnosis of the ordering is wrong and the block is somewhere I have
not looked. (5) **On deploy:** if `gsc.tracked` shows county impressions in the
thousands rather than the low hundreds, the flood is a smaller share of the
...
`check_coverage()` only ever asks one page whether it targets a query — the page
named in that keyword's `target` field (`keywords.py:285`). Nothing fills that
field in. `add()` defaults it to `""` (`keywords.py:148`), the scout supplies
nothing, and the only writer is `strengthen_pages`, which sets it as a *side
effect of writing a new section*, one query per day (`techniques.py:1075`).

So every query the scout has ever added reads "no page targets this query"
regardless of what the site actually says. Today that is **13 of the 128
uncovered queries that an existing page already targets in its own title or h1**,
verified by running the new matcher against the live pages:

| uncovered query | page that already targets it |
|---|---|
| copper gutters york pa cost | /guides/copper-gutters-historic-home-york-pa.html |
| gutter cleaning york pa cost | /guides/gutter-cleaning-cost-york-pa.html |
| leaffilter cost york pa | /guides/leaffilter-alternative-york-pa.html |
| gutter guard cost pa | /guides/leaffilter-alternative-york-pa.html |
| gutter pulling away from house repair (+ " cost") | /guides/gutter-pulling-away-from-house.html |
| emergency gutter repair near me york pa (+ "york county pa") | /services/emergency-gutter-repair.html |
| half round gutters near me | /services/half-round-gutters.html |
| best gutter company in york county pa | /guides/best-gutter-company-york-county-pa.html |
| 5 inch vs 6 inch gutters which do i need (+ "which is better") | /guides/5-vs-6-inch-…-york-county-homes.html |
| seamless gutter company spring grove pa | /areas/seamless-gutters-spring-grove-pa.html |

**This one mechanism explains three numbers I have been reporting separately for
three weeks as if they were three problems.**

1. **`coverage_pct` drifting down** — 33.5 → **32.6** today, the first fall that
   is not rounding. The scout adds up to 12 queries a day, all uncoverable by
   construction; the engine covers about one. The denominator was always going to
   win.
2. **`by_intent.price.covered` frozen at 3** while price total went 35 → 37 → 38
   → 42 → **46**. Five of the 13 rows above are cost queries. They could never
   have been counted.
3. **`strengthen_pages` spending its one daily write appending sections to pages
   that already answer the query.** That is the part that costs actual output.

**I was wrong yesterday about (2) and I want that on the record.** On 08-16 I
attributed the frozen price number to intent ordering at `techniques.py:966`,
wrote a round-robin fix, and predicted price would move off 3 within a week of
deploy. The ordering is real and the fix is fine, but it was not the cause: with
perfect ordering, price queries still arrive permanently uncovered. I diagnosed
the queue and the bug was in the ruler.

### Where the numbers stand

**The goal metric: `top3` = 2 of 190 tracked queries, `share_pct` 1.1%. The count
of 2 has not moved since 08-10 — eight days.** Target 50%.
...
| dover | 16 (=) | 6 (=) | 0 |
| red-lion | 11 (+1) | 3 (=) | 0 |
| dallastown | 11 (+1) | 4 (+1) | 0 |
| spring-grove | 11 (+1) | 3 (=) | 0 |
| hanover | 9 (=) | 4 (=) | 0 |

**Zero top-3 positions in all seven named towns, day twenty-two.** `top10` 7 and
`ranked_known` 23, both unmoved for over a week. Tracked +8, covered +1 — an 8:1
loss rate, worse than yesterday's 3:1, and see the lead for why that ratio is
partly an artefact.

**Search Console: 926 rows / 23 matched / 16 clicks / 26,011 impressions / avg
position 25.0.** `matched` stuck at 23 for the **tenth** day. Position 24.8 → 25.0.
Clicks +2. Row growth **+73**, after +7, +43, +147, +6 — the jumpiest series here.

**The flood rows are frozen for a fourth consecutive day** — I diffed
`discovered_untracked` against yesterday's published snapshot: 0 added, 0 removed,
0 changed, same 40 rows, same 12,067 impressions, same 0 clicks, `gutter installer`
still exactly 471 at position 1. Consistent with an event that ran ~08-04 → 08-11
and finished.

**But total impressions rose +1,040 with those rows frozen, so something else is
accruing** — 13,944 impressions live outside the 40 flood rows, and I cannot see
them: 926 rows exist, the snapshot publishes 23 matched plus 40 discovered = 63.
That has a consequence for my own prediction; see below.

**I also closed off the worst explanation of the flood.** If the site's own pages
named those Philadelphia suburbs, "someone else's SERP" would be wrong and this
would be self-inflicted. I grepped every `*.html` for glenside, audubon, plymouth
meeting, royersford, eagleville, horsham, phoenixville, norristown, abington and
wayne pa: **zero hits.** The external-event reading stands.

**Traffic. Third beacon day: 2 visitors, 2 pageviews, 0 organic, 2 direct.** The
series now reads 2, 2, 2 on 08-14/15/16 with pageviews 2, 2, 2 — pages-per-visit
exactly 1.00 for three days. `analytics.js:50-70` fires the pageview IIFE on every
load and `metrics.py:555` increments per hit, so a two-page session should log two.
I confirmed the beacon is on **all 39 pages**. At n=2 this proves nothing; the
first honest 7-day window still closes **2026-08-21**, and `log_visitors` is still
undeployed so I still cannot cross-check the beacon against the raw log.
`ai_visitors` is **0 for all 23 days on record**.

**Leads.** `bookings_all_time` **3**, `phone_leads_all_time` **0**, `call_taps`
**1 all-time** (08-12, no second tap in five days), `ai_calls` 0 since 07-30.
Nothing new.

**Ledger: 71 techniques — 11 active, 1 retired, 59 candidates.** The pile grew by
3 again. **`scoreboard.does_not_work` is `[]` on day twenty-two: zero techniques
activated by a human, zero rejected, ever.**

**The engine is working.** `last_build` is clean, 2 pages changed, and `last_scout`
is `ok: true`. **My standing instructions are still stale on this** — they tell me
to expect a scout failure on a spend cap "until 2026-08-01", seventeen days ago.
I flagged this on 08-16 and it has not been corrected. Per Step 3b the data wins.

**One recommendation closed itself: the engine fixed the homepage title.** Day 17
of me asking, and `improve_ctr` did it this morning — `index.html:16` is now
`Gutter Contractor & Cleaning York PA | NEMO Seamless Gutter` (57 chars). It chose
the wording from real impression data (1160 impressions, 9 clicks, position 5.5),
which is better than the copy I proposed. **Dropping it from the list.**

### Did previous changes work?

**Prediction 1 — 7-day median `organic_visitors` ≥ 2 by 08-16. Passed, closed.**
The window 08-10→08-16 is 2, 3, 2, 2, 2, 1, 0 — median 2. The final day came in at
**0**. Repeating the caveat because it is the whole lesson: this was a prediction
about whether a number that oscillates 0-3 lands on 2. It is not evidence that
anything worked, and I will not make another of that shape.

**Prediction 2 — `call_taps` > 0 by 2026-09-01. Resolved yes on 08-13.** Still 1
all-time. Five days, no second tap.

**Prediction 3 — the impression flood. Not yet testable, and I am revising the
test because today's data shows it was badly specified.** I predicted total
impressions falling to 6,000-9,000 by 09-13 as the 08-04→08-11 burst leaves the
28-day window. Today total impressions rose +1,040 while the flood rows sat
frozen. If ~1,000/day keeps accruing from rows I cannot see, the total will not
fall to 9,000 even if the flood exits exactly as predicted, so the test would fail
for a reason unrelated to the claim. **Restated at row level, where I have clean
data: the 40 flood rows should disappear from `discovered_untracked` between
2026-09-01 and 2026-09-08.** If they are still there on 09-08, the burst is
recurring and the event reading is wrong.

**Prediction 4 — `keywords_added` < 5 after the scout change deploys. Ninth day,
still not deployed, not evaluable.** Today `keywords_added` = 8.

**Prediction 5 — `by_intent.price.covered` moving off 3. Failed again: 3 of 46.**
Cause identified and it is not what I said yesterday. See the lead.

**Yesterday's `techniques.py` round-robin (commit `313b2ff`): committed, not
deployed, and now tested.** I said on 08-16 that pytest was not installable in
this container and someone should run the suite before deploying. **`pip install
pytest` worked today and the full suite passes: 212 tests, including the round-robin
change and the two I added.** That blocker is cleared.

**The deploy — day 10 for `snapshot.py`.** Re-verified against today's file:

| what | in `main` since | visible in today's snapshot? |
|---|---|---|
| `gsc.tracked` (`9479b5c`) | 08-04, **13 days** | no |
| `gsc.pages` (`snapshot.py:208`) | before 08-09 | no |
| `crawl_*` series (`727deec`) | 08-07 | no |
| `code_version` (`snapshot.py:231`) | 08-09 | no |
| `keywords.ranked` (`snapshot.py:200`) | before 08-09 | no |
| `log_visitors` (`9e18cbf`) | 08-15 | no |

**Not actioned, with day counts:** `growth/` deploy **day 10**; PA HIC registration
**day 3**; the live Schuylkill passages **day 8**; the Akron paragraph **day 20**;
T016 GBP category **day 22**; T007 reviews **day 22**; T033 review replies **day
6**; T011 LSA decision **day 20**; `?utm_source=gbp` **day 15**; the goal
denominator **day 10**. Re-verified in the working tree today:
`services/seamless-gutter-installation.html:227-236` is still an entire
Schuylkill County section — an h2, prose naming Pottsville, Schuylkill Haven,
Orwigsburg, Frackville, Minersville, Tamaqua, Pine Grove, Ashland and Mahanoy
City, and an FAQ that answers "Do you actually service Schuylkill County" with
"We service both". `services/gutter-guards.html:163` still opens with "homes in
Akron, PA and the surrounding Lancaster and York County area" — that is the
page's lead paragraph, not a buried line. A grep of every `*.html` for
`HIC #|home improvement contractor|PA #|licensed and insured|fully insured`
returns **zero hits** for the third day.

**Line 199 of the installation page is fine and should be left alone** — it names
Harrisburg and Lancaster as a *contrast* ("an estimate doesn't mean waiting on a
crew coming down from Harrisburg"). I checked before recommending a deletion so
nobody deletes good copy.

**As established on 08-16, I cannot fix any live page from this repo.**
`publish_state.sh` copies droplet → repo only, so an edit here is overwritten by
the next 06:00 publish. `growth/*.py` is the exception and is where both of
today's changes live.

### What I researched today

**1. The free call button is disappearing from organic local-pack listings, and
the ads keep theirs. This is the most consequential thing I have found in three
weeks, because it breaks the causal chain every recommendation in this journal
rests on.** Sterling Sky reports that clients running ads now get better ROI
"since the ads have better placement and contain features (like call buttons)
that the organic listings are losing", and that the local pack ad unit
"exclusively displays action buttons"
([Sterling Sky](https://www.sterlingsky.ca/the-ultimate-guide-to-local-pack-3-pack-ads/),
[State of Local SEO 2026](https://www.sterlingsky.ca/the-state-of-local-seo-in-2026/)).
Local-pack ads went from ~1% of tracked keyword packs to ~22%; Local Services Ads
from 11% to 31% of tracked queries
([ppc.land](https://ppc.land/google-increases-local-pack-ads-by-733-in-three-months/),
[Search Engine Land](https://searchengineland.com/local-rankings-fine-calls-vanishing-468321)).
Affected owners are reporting it directly on Google's own forum
([GBP Community thread](https://support.google.com/business/thread/405819728/call-button-no-longer-showing-on-organic-map-pack-local-listings?hl=en)).
Separately, AI local packs surface roughly 32% as many unique businesses as the
legacy 3-pack and show 1-2 rather than 3
([Sterling Sky](https://www.sterlingsky.ca/the-state-of-local-seo-in-2026/),
[Marketing Code](https://www.marketingcode.com/ai-search-overview-local-pack-map-pack-contractor-citation-playbook-jul-2026/)).
**Caveat on the dates: retellings disagree on the baseline** — one gives Nov 2024
→ 22%, another early-2025 → Dec-2025 22%. The direction is consistent across
sources; treat the exact start month as approximate.
**What this changes for NEMO:** "fix the profile → rank in the pack → the free
call button rings Eric" now has a documented break in the middle. The profile work
is still right — it is how you get found at all, and it is what Ask Maps reads —
but **it should be judged on profile views and website clicks, not on calls**, and
the surface that still reliably carries a call button is the paid one. That is
evidence for a decision on T011 that has been open twenty days, and it also means
the site's own `tel:` links matter *more*, not less, because profile → website →
call is the path that survives.

**2. Reviews moved from 16% to 20% of local-pack weight, and recency now beats
count.** Whitespark's 2026 report puts GBP signals at 32%, reviews at **20%** (up
from 16% in 2023), on-page 19%. Darren Shaw ranks review *recency* in his personal
top 5; reviews inside 90 days outweigh older ones regardless of volume; reviews
that name a specific service carry more weight than generic ones; and businesses
answering 80%+ of reviews see measurable ranking improvement
([Whitespark](https://whitespark.ca/local-search-ranking-factors/),
[w3marketinghub](https://w3marketinghub.com/seo/local-seo-ranking/),
[Reputation](https://reputation.com/resources/articles/whitespark-2026-three-insights-every-brand-should-know)).
Multiple 2026 write-ups also describe a shift from "prominence" (history,
backlinks, total review count) to "popularity" (current clicks, calls,
interactions)
([Blue Interactive](https://blueinteractiveagency.com/seo-blog/2026/06/google-business-profile-optimization-in-2026/),
[Emarketed](https://emarketed.com/seo/local-seo-2026-service-business-rankings/)).
**For NEMO:** 13 reviews at 4.2 stars, and nothing in this journal suggests any of
them are recent. Under a recency-weighted model a stale 13 is worth much less than
it looks, and this is the second entirely free lever after the profile fields.
It raises T007, T033 and T047 together.

**3. Profile staleness has a reported cliff at about 30 days.** Several 2026 guides
report meaningful impression drops for profiles that go 30+ days without a new
photo or update
([Explore Digital](https://www.exploredigital.com/blog/top-8-biggest-changes-to-google-business-profile-in-2026-so-far/),
[Projul](https://projul.com/blog/google-business-profile-contractors-guide),
[Raven SEO](https://raven-seo.com/google-business-profile-for-contractors/)).
**I am deliberately reporting the floor, not the prescription.** The same sources
tell contractors to post "twice a week" and to add "20 geo-tagged photos"; the
frequency numbers are agency advice with an obvious incentive behind them, and
geo-tagging photos for ranking purposes is a long-debunked myth that my own 08-16
research recorded 2026 sources listing as noise or risk. The defensible version
is: **do not let the profile go dark for a month.** Real photos of real jobs,
whenever there is a job.

**Rejected today.**
- **T071, added by the scout this morning — reject outright, the page exists.**
  Its hypothesis says of copper and half-round work "there is no page for it."
  `/guides/copper-gutters-historic-home-york-pa.html` (19.7 KB) and
  `/services/half-round-gutters.html` (42.3 KB) are both live and in the docroot
  inventory. The guide carries an h1 on copper for historic York County homes, an
  h2 of 2026 per-linear-foot price ranges, a material comparison and a
  historic-district-approval FAQ; the service page carries its own "Copper
  Half-Round Gutters for Historic Homes in York, PA" section and a HARB review
  FAQ. T071's stated first step — "write the page" — is work already done.
  Worse, its own falsification is already in: those pages have been live and
  copper queries are **not** in the top 3, which is direct evidence against its
  central claim that low-competition material queries are the fastest route
  there. This is the exact failure my standing instructions name, arriving from
  the scout instead of from me, so I fixed the cause rather than just declining
  it — see below.
- **FAQPage structured data**, which the AEO sources push every week.
  `techniques.py` already emits FAQPage JSON-LD on every page it generates.
  Already shipped; checked again.
- **Filing today's research as new techniques. I filed nothing.** 59 candidates,
  zero ever picked. Adding a sixtieth is not analysis.
- **T069 Microsoft Ads is not rejected, but it should not go first.** It and T011
  compete for the same small budget, and today's research says Google is actively
  privileging the surface T011 sits on. Sequencing note only.

**A standing limitation worth recording.** The egress proxy blocked every primary
I tried to fetch — searchengineland.com, sterlingsky.ca, whitespark.ca, ppc.land,
scaledon.com, exploredigital.com. `curl "$HTTPS_PROXY/__agentproxy/status"` shows
no relay failures, so this is the environment's domain policy, not a broken
connection. Today's findings therefore rest on search-result summaries plus
agreement across independent domains, not on primaries I read. Same caveat as
yesterday, now clearly recurring: **if verified sourcing matters, the network
...
**Nothing in this commit is live.** The site runs from
`/var/www/nemo-seamless-gutter`, which is not a git checkout, and
`publish_state.sh` copies droplet → repo only. Both of today's code changes need
a deploy before they do anything.

1. **Eric: twenty minutes on the Business Profile — primary category, then the
   service-area list, then the itemised Services.** *(Free. T016/T051/T022, day
   22.)* Seventh consecutive day at the top. GBP signals are 32% of pack weight
   with primary category the largest single one, and since Ask Maps replaced the
   Q&A section these same fields are the text Gemini answers York County gutter
   questions from. Screenshot first, change one field at a time, count the service
   areas — padding is a documented negative. **Skip the "Owner Q&A" half of T016;
   that feature no longer exists.** *Also start a photo habit:* a real photo from
   a real job whenever there is one, so the profile never goes 30 days dark.
   *How you would know:* the `york` bucket moves off 5/0, or `local_visitors`
   lifts off ~0, within a month. **Judge it on profile views and website clicks,
   not on calls** — see research item 1. *Checked:* T016, T051 and T022 are all
   `status: candidate, activated: null` in today's snapshot.
2. **Eric: reply to all 13 reviews in one sitting, then ask every finished job for
   one.** *(Free, no customer contact for the replies. T033 day 6, T007 day 22.)*
   Promoted today on new evidence: reviews are now 20% of pack weight, up from
   16%, recency inside 90 days outweighs volume, owner-response rate above 80%
   shows measurable improvement, and reviews naming a service and a town weigh
   more than generic ones. A stale 13 is worth much less than it looks. **Neutral
   wording, everyone asked, good or bad** — the FTC rule in force since October
   2024 carries penalties reported up to $51,744 per violation for incentivised
   or gated reviews, so no gift cards, no filtering, no "only if it's five stars".
   *How you would know:* review count and the date of the newest review, both
   visible on the profile. *Checked:* T033 and T007 are both `candidate`/
   `activated: null`.
3. **Divine: deploy `growth/`.** *(Minutes. Day 10.)* Now blocking eight things:
   `gsc.tracked` (13 days), `gsc.pages`, `crawl_*`, `code_version`,
   `keywords.ranked`, `log_visitors`, yesterday's round-robin, and today's two
   fixes. Ship `scout.py` + `growth_daily.py` **as a pair** or the scout raises
   `TypeError`. **The test blocker I raised on 08-16 is cleared — 212 tests pass
   locally, including the round-robin.** *How you would know:* the next snapshot
   carries `traffic.log_visitors`, `gsc.tracked`, `keywords.ranked` and
   `code_version`, and `coverage_pct` jumps (see prediction 1).
4. **Eric: confirm the PA HIC registration number and put it on the site, the
   Business Profile and the estimate form.** *(Free if already registered. Day 3.)*
   Unchanged. PA law requires the registration number on advertising distributed
   in Pennsylvania and a website is advertising; this is the only item here that
   is a legal exposure rather than a growth idea. Trust marks beside contact
   buttons are also a documented ~12.6% conversion lift, and this site carries no
   "licensed", "insured" or registration line anywhere. *If Eric is not registered
   and does ≥$5,000/year, that is the finding and it outranks everything above.*
   *Checked:* grepped every `*.html` again today — zero hits.
5. **Divine: delete the Schuylkill County section and rewrite the Akron lead.**
   *(Ten minutes, free. Days 8 and 20.)*
   `services/seamless-gutter-installation.html:227-236` — remove the h2, the
   nine-town prose and the "Do you actually service Schuylkill County" FAQ
   outright. `services/gutter-guards.html:163` — "homes in Akron, PA and the
   surrounding Lancaster and York County area" → "homes across York County,
   Pennsylvania". **Leave line 199 alone**, it names Harrisburg and Lancaster as a
   contrast and it is good copy. Marked Divine because a commit here would be
   reverted by the next publish. The guards that prevent recurrence
   (`_off_area_prose` `techniques.py:1413`, `SERVICE_AREA_WORDS`
   `techniques.py:1328`) are in git and ship with the same pending deploy.
   *Why it matters more this week:* a service-area business with zero top-3
   positions in its own county has two service pages claiming counties 60+ miles
   away.
6. **Eric: make the Local Services Ads decision. T011, day 20.** *(Costs money —
   his call, and I am not spending it.)* This is a decision I have been listing
   as "open" for twenty days without new information. Today there is some: the
   free call button is being withdrawn from organic pack listings where ads run,
   ads keep theirs exclusively, and LSA now appears on 31% of tracked queries
   against 11% a year ago. That does not make LSA correct for a one-truck
   operator — it makes "wait and see" more expensive than it was. Either commit a
   capped monthly number before leaf-fall or reject T011 and stop carrying it.
   **A rejection is a real answer and would be the first entry ever in
   `does_not_work`.** *Checked:* T011 is `candidate`/`activated: null`.
7. **Someone must decide what the goal metric counts. Day 10, and today it went
   backwards.** `top3` has been 1-2 since 08-01 while `tracked_queries` went 108 →
   **190**. `coverage_pct` fell 33.5 → 32.6 — the first real fall, not rounding.
   Either cap the tracked universe or promote `top3 / ranked_known` (2 of 23) to
   the headline. **I have not made this change myself: it redefines the number
   Eric is judged against.** Note today's coverage fix moves `coverage_pct` up
   without touching this — they are different problems and fixing one does not
   answer the other.
8. **Eric: pick three candidates and reject the rest — starting with T071, which
   should be rejected as already-built.** *(Day 10. Pile now 59, up 3 today;
   `does_not_work` still `[]` after twenty-two days.)*

**On cadence.** On 08-16 I said that if this list is still untouched on
**2026-08-21** the review should drop to weekly and shrink to three lines, with
the HIC item exempted as a legal matter. **I stand by that. Four days.**

### What I changed in this repo today

**Two code changes, both in `growth/`, both tested. 212 tests pass** (`pip install
pytest` works in this container now — it did not on 08-16).

**1. `growth/keywords.py` — coverage falls back to the site when a keyword has no
usable target.** New `_headline_index(docroot)` builds a title+h1 map of all 41
pages once per `check_coverage()` call; new `_find_host()` credits a query to a
page whose title/h1 carries **all** its distinguishing tokens. Rationale in the
lead. The three guards that keep it honest:
- **Title and h1 only, never h2.** `strengthen_pages` appends an h2 per run, so
  the area pages have accumulated headings that match almost anything. Measured
  on today's list: h2 matching flips 43 of 128 and its picks include "best gutter
  company york pa" → the *Dallastown* page. Title and h1 are what a page is about.
- **At least two distinguishing tokens** (`FALLBACK_MIN_TOKENS = 2`). One token is
  not aboutness. Today's single-token matches were "how much to replace gutters on
  a house" → the soffit-and-fascia page (wrong) and "gutter cleaning near me york
  pa" → three pages at once. At two tokens every surviving match is defensible and
  both of those are excluded. Deliberately conservative: a missed match costs a
  day of writing, a false one marks the goal metric covered when nothing covers it.
- **An assigned `target` still wins.** The fallback only fires when `target` is
  empty or points at a file that is not there, so seeded keywords behave exactly
  as before.
It does **not** write `target` — assignment stays with `strengthen_pages`, which
does it deliberately. The matched page is recorded in `coverage_detail` so this is
auditable rather than magic. Seven new tests, including the wrong-page case, the
body-copy-only case, a missing docroot, and one asserting the index is built once
rather than per keyword.

**This raises a published number and I want that flagged rather than buried:
`coverage_pct` goes from 32.6% to at most 39.5% on deploy** (13 of 128, upper
bound — I cannot see from here which uncovered queries have an unresolvable target
versus none at all, and only the latter are affected). I judged this a bug fix
rather than a redefinition, because `check_coverage`'s own docstring defines
coverage as "does a page exist whose title/h1 actually targets it" and it was
answering no for pages that plainly do. That is a different thing from
recommendation 7, which is about the goal metric and is not mine to change.
**If Eric or Divine disagrees, revert this one commit** — the diagnosis in the
lead stands on its own and does not depend on the fix shipping.

**2. `growth/scout.py` — the scout is given page names, not page counts.**
`_page_counts()` → `_page_names()`, plus `_page_block()` to render them, plus an
explicit instruction not to propose a page whose subject is already listed and to
propose strengthening the named page instead. T071 is why: a count tells a writer
the site is substantial, not which sixteen guide subjects are taken. Five tests
updated, three added.

I did not touch `techniques.json`, `keywords.json`, `results.jsonl` or
`state.json`, did not activate, retire or re-status any technique, and made no
page-copy or keyword-list edits — as established, page copy committed here is
reverted by the next publish anyway.

### Reasoning and uncertainties

Day twenty-two. Two top-3 queries county-wide, zero in all seven named towns,
zero AI visitors across twenty-three days, zero phone leads ever, one call tap,
three bookings, ~2 human visitors a day. **None of that moved today.** What moved
is that one of the numbers I have been using to reason about all of it turned out
to be partly manufactured, and that the mechanism by which good rankings become
phone calls has a documented hole in it that did not exist a year ago.

Those two findings point opposite ways and I should say so plainly. The coverage
bug is the kind of thing I am well placed to find — it is in the repo, it is
falsifiable, and I fixed it in an afternoon. It is also **worth close to nothing
in revenue.** Coverage is a proxy for a proxy; correcting it from 32.6% to 39.5%
does not put a single person on the site. The call-button research is the opposite:
I cannot verify it from primaries, I cannot act on it at all, and it matters far
more than anything in my diff. **Twenty-two days of free profile work and free
review work remain untouched while I improve the instruments.** If this pattern
holds another week, the honest conclusion is that this review is producing
engineering, not growth, and the cadence change I committed to on 08-21 should
happen without argument.

**Where I am least confident. Five things.**

*First, the coverage fix could be over- or under-tuned and I cannot tell from
here.* The two-token rule is a judgement call fitted to one day's data — today's
128 queries. It might be too strict: "5 inch vs 6 inch gutters" reduces to the
single unique token `inch` and stays uncovered even though the dedicated guide
plainly targets it. It might be too loose on a query I have not seen. The
falsifiable version is prediction 1.

*Second, I do not know how many of the 128 have an unresolvable target versus no
target at all*, which is why 39.5% is an upper bound and not an estimate. This is
the fourth consecutive entry where the answer is in `keywords.json` on the droplet
and I cannot read it. `keywords.ranked` would show me — it has been in `main`
since before 08-09.

*Third, the call-button finding rests on secondhand text.* Six domains agree and
one is Google's own support forum, so I believe the direction. But the proxy
blocked every primary, the baseline dates disagree between retellings, and the
strongest source is a consultancy that sells ad management and therefore benefits
from the conclusion. I would not want a spending decision made on my summary
alone; Divine or Eric should read Sterling Sky's post directly before committing
money to recommendation 6.

*Fourth, the +73 rows and +1,040 impressions.* Row growth has gone +147, +43, +6,
+7, +73 — I have no model for that, and a second geographic burst would look
exactly like today. Prediction 3 below is the tripwire.

*Fifth, unchanged and now three weeks old: I cannot judge the quality of the
generated copy.* I can now see that the copper guide has sensible headings and
real per-foot prices, which is mild evidence for the generator. But "the pages
have good headings" is not "the pages are worth reading", and if someone reads
five guides and finds them thin then "on-site work is saturated" collapses.

**What would change my mind, dated.**
1. **On deploy of today's `keywords.py` change:** `coverage_pct` should land
   between 37% and 40%, and `by_intent.price.covered` should move from 3 to
   roughly 6-8 in the first snapshot after deploy. **If price is still 3 after
   this ships, I have now been wrong twice about the same number and the block is
   somewhere I have not looked.**
2. **2026-08-21:** the first honest 7-day beacon window closes. Medians 2-3 and
   the ~2-4-humans-a-day reading is confirmed; medians 6+ and I over-read three
   quiet days.
3. **2026-09-08:** the 40 flood rows should be gone from `discovered_untracked`.
   Still present and the burst is recurring, not an event. **Separately, if
   `gsc.rows` grows by more than 50 on three of the next five days, a second
   burst has already started** and the "finished event" reading needs revisiting
   before 09-08.
4. **On deploy:** if `log_visitors` reads ~12 against a beacon 2 on the same day,
   the beacon story holds; if it reads ~3, the old log filter was roughly right
   and 08-16's lead was wrong.
5. **If pages-per-visit is still exactly 1.00 after seven beacon days**, the
   beacon fires once per session and the pageview series means less than I have
   assumed.
6. **On the profile:** if Eric does recommendations 1 and 2 and the `york` bucket
   is still 5/0 with zero new calls a month later, then the map pack is not the
...
## 2026-08-19 — engine run

Goal: **1.0%** top-3 share of 195 tracked queries (target 50%).
2026-08-18: 2 visitors (0 organic, 0 maps) · 0 bookings, 0 phone leads.

**Built:**
- `adopt_queries` — ok: no new in-area searches worth tracking
- `improve_ctr` — ok: no page is due a snippet rewrite
- `geo_answer_first_content_pass` — ok: every ranking page already opens with a direct answer
- `strengthen_pages` — FAILED: 3 candidate(s) failed, last: generation failed for 'half round copper gutters york pa': anthropic 400: {"type":"error","error":{"type":"invalid_request_error","message":"Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits."},"request_id":"req_011CeBbZiabi26ezb6ZqQoDU"}
- `service_pages` — ok: every queued service already has a page
- `area_pages` — ok: every queued town already has a page
- `money_pages` — FAILED: content generation failed for 'instalacion de canaletas york pa': anthropic 400: {"type":"error","error":{"type":"invalid_request_error","message":"Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits."},"request_id":"req_011CeBbZjdbTqhwpWU6YjmKx"}
- `internal_links` — ok: refreshed nearby-links on 0 page(s)
- `local_schema` — ok: LocalBusiness schema already current
- `rebuild_sitemap` — ok: sitemap.xml unchanged
- `ping_indexnow` — ok: nothing new to submit

**Scout did not run:** anthropic 400: {"type":"error","error":{"type":"invalid_request_error","message":"Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits."},"request_id":"req_011CeBbZmoa3D13VfTrjbMgW"}

## 2026-08-19 — review agent

### Lead: the engine broke this morning, and the goal metric stopped falling. Those are the same event, and it is the cleanest evidence yet that the metric is measuring the scout, not the business

The Anthropic credit balance emptied again overnight. `strengthen_pages` and
`money_pages` both failed, and the scout did not run at all — all three with
`"Your credit balance is too low to access the Anthropic API"`.

And `coverage_pct` held at **32.3%** and `tracked_queries` held at **195** —
the first flat day since 08-13.

I pulled every published snapshot back to 08-04 and checked the scout's status
against the denominator it feeds:

| date | scout ok | tracked | Δ | coverage_pct |
|---|---|---|---|---|
| 08-04 | yes | 127 | — | 38.6 |
| **08-05** | **no** | **127** | **0** | 38.6 |
| 08-06 | yes | 138 | +11 | 37.7 |
| 08-07 | yes | 140 | +2 | 37.9 |
| 08-08 | yes | 146 | +6 | 37.0 |
| 08-09 | yes | 152 | +6 | 36.2 |
| 08-10 | yes | 161 | +9 | 34.8 |
| 08-11 | yes | 161 | 0 | 35.4 |
| **08-12** | **no** | **161** | **0** | 35.4 |
| **08-13** | **no** | **161** | **0** | 35.4 |
| 08-14 | yes | 170 | +9 | 34.1 |
| 08-15 | yes | 176 | +6 | 33.5 |
| 08-16 | yes | 182 | +6 | 33.5 |
| 08-17 | yes | 190 | +8 | 32.6 |
| 08-18 | yes | 195 | +5 | 32.3 |
| **08-19** | **no** | **195** | **0** | **32.3** |

**Four days out of four where the scout failed, the denominator did not move.
Ten days out of eleven where it ran, the denominator grew — mean +6.2/day.**

Yesterday I inferred the denominator problem from a twelve-day trend. Today it
is a controlled experiment I did not have to design: the treatment was applied
four times by an unrelated billing failure, and the response is 4/4 in the
predicted direction. **`coverage_pct` and `share_pct` are, to a first
approximation, functions of whether the scout ran.** That is not a metric.

I want to be exact about what this does *not* show. It does not show the scout
is adding junk — the queries may all be real York County demand. It does not
show the site is doing well. `top3` is still **2**, tenth consecutive day, and
that number is untouched by the denominator argument at the numerator end.
What it shows is that the *reported percentage* is dominated by scout uptime,
so no one should read a change in it as a change in the business.

**The billing failure itself is the third episode, and it has self-healed
twice.** 08-05 failed and 08-06 was clean; 08-12 and 08-13 failed and 08-14 was
clean. Nobody intervened either time as far as this journal records. So I am
**not** raising this as an emergency, and I am specifically not asking anyone to
go top up an account at 11am on a Wednesday. If 08-20 is also failing, that is a
different story and it breaks a pattern rather than repeating one. The spacing
(08-05, 08-12, 08-19) is exactly seven days, which is either a weekly billing
cycle worth someone's five minutes or an n=3 coincidence; I cannot tell from
the repo, and I am flagging it rather than diagnosing it.

**The cost of today's outage is genuinely small, and that is its own
indictment.** Nine of eleven build steps reported `noop` — "every queued town
already has a page", "every queued service already has a page", "no page is due
a snippet rewrite". The two that failed were trying to write a half-round copper
page and a Spanish-language page. The engine lost about two pages it has not
been able to write for weeks anyway.

### Where the numbers stand

**Goal metric: `top3` = 2 of 195, `share_pct` 1.0% against a 50% target.
Flat at 2 for the tenth consecutive day.** `top10` 7, unchanged.
`ranked_known` **23 → 25**. **Zero top-3 positions in all seven named towns,
day twenty-four** — hanover 0/10, dover 0/16, red-lion 0/11, dallastown 0/11,
spring-grove 0/11, york 0/33. Both top-3s remain in the county bucket, 2 of 103.

**Search Console: 935 rows / 25 matched / 18 clicks / 26,144 impressions /
avg position 25.0.** Against yesterday: rows +6, clicks +1, impressions +70,
avg_position flat. **`matched` moved for the first time since 08-08, 23 → 25** —
and because the scout was dead, the tracked universe did not grow underneath it,
so this is two tracked queries the site newly appears for rather than an
artefact. It is the only genuinely positive number in today's snapshot, and it
is two queries.

**Coverage by intent is still lopsided the same way:** hire 55/118, check 4/22,
diy 1/8, **price 3 of 47**. The price lane is the one a homeowner with a sagging
gutter actually searches, and it is 6% covered.

**Traffic, fifth beacon day: 2 visitors, 2 pageviews.** The five beacon days
are 2, 2, 2, 3, 2. **Pages per visit is exactly 1.00 on all five** — eleven
sessions, eleven pageviews, still not one person has viewed a second page.

**Leads. `bookings_all_time` 3, `phone_leads_all_time` 0.** `call_taps` still
**1 all-time**, from 08-12 — **seven consecutive days with no call tap**.
`ai_visitors` 0 across twenty-five days.

**Ledger: 74 techniques — 11 active, 1 retired, 62 candidates.** Unchanged
today, because the scout that generates candidates was dead. `does_not_work` is
still `[]` on day twenty-four.

**Deploy: day 12.** `code_version`, `traffic.log_visitors`, `gsc.tracked`,
`gsc.pages`, `keywords.ranked` all still absent from the snapshot. The droplet
is running pre-08-08 code.

### Did previous changes work?

**Prediction 1 — deploy lands `coverage_pct` at 39.5% and `price.covered` at 8.
Still untestable, day 12.** All five deploy tells absent; `price.covered` reads
3. Unchanged status, and I am not re-deriving the estimate a third time.

**Prediction 2 — 7-day beacon window closes 08-21. Five of seven days in:
2, 2, 2, 3, 2.** Median 2. Nothing contradicts the ~2–3-humans-a-day reading and
the last two days will not overturn it.

**Prediction 3 — flood gone from `discovered_untracked` by 09-08. I now think
this prediction is wrong, and I want that on the record before the date
arrives rather than after.** Today the 40 rows moved by **exactly one
impression on one row** ("gutter installer", 471 → 472); the other 39 are
byte-identical for a sixth day. I read that as a *rolling-window* artefact, not
a static block: `gsc.py:45` sets `WINDOW_DAYS = 28`, so a source polling these
queries at a constant daily rate produces impressions that are flat inside the
window, not decaying. **If that is right, the flood does not age out on 09-08 or
any other date — it persists until the poller stops.** My original reasoning
assumed a one-off historical burst; a burst would decay, and it isn't decaying.

The rank-tracker reading itself got stronger, from one row: **"gutter installer",
position 1.0, 472 impressions, 0 clicks.** A genuine position-1 result in front
of 472 human searches gets clicks. Zero is a machine. The other 39 are all
Philadelphia-ring towns — Glenside, Audubon, Wayne, Plymouth Meeting,
Royersford, Eagleville, Horsham, Phoenixville, Norristown — none in York County.

The second-burst tripwire (rows +50 on three of five days) has not fired: +6
today.

**Prediction 4 — `log_visitors` ~12 vs beacon ~2. Blocked on the deploy, day
12.**

**Prediction 5 — pages-per-visit still 1.00 after seven beacon days means the
series carries no information. Five of five.** I said yesterday I would not act
on this before ~50 sessions and I still won't; see research item 3, which gives
it a reading I had not considered and which makes it worse rather than better.

**Prediction 6 — the profile lever. Untestable; recommendations 1 and 2 have
not been done.**

**Not actioned, with day counts.** `growth/` deploy **day 12**; GBP category /
service-area / services **day 24**; review replies **day 8** (T033) / **day 24**
(T007); PA HIC registration **day 5**; the Schuylkill passages **day 10**; the
Akron paragraph **day 22**; the LSA decision **day 22**; the goal-metric
definition **day 12**; candidate triage **day 12**.

I re-verified the four free items against the live files this morning rather
than trusting yesterday's note: `services/seamless-gutter-installation.html:227-230,235`
still carries the Schuylkill County section and its FAQ;
`services/gutter-guards.html:163` still opens with "homes in Akron, PA and the
surrounding Lancaster and York County area"; and a fresh grep of all 41 HTML
files returns the same two hits for "licensed"/"insured" —
`services/seamless-gutter-installation.html:243` and
`guides/gutter-services-near-me.html:181` — both of them checklists telling the
reader to demand credentials NEMO never states about itself.

### What I researched today

**1. The Business Profile category recommendation now has a number on it, and
it is bigger than I have been claiming.** Whitespark's 2026 Local Search Ranking
Factors report puts the **primary category at 227 ranking weight — the single
heaviest individual factor in the local pack — and a wrong primary category at a
214-point penalty.** So the category field is not merely the largest positive
input; picking one notch too broad actively pushes a profile down. GBP signals
are ~32% of pack weight overall, ahead of on-page (19%), reviews (16%) and links
(15%), and **8 of the 10 top pack factors come straight off the profile.**
- https://whitespark.ca/local-search-ranking-factors/
- https://anthonylouismedia.com/google-business-profile-categories-explained-the-complete-guide-for-home-services-businesses/
- https://wolfpackadvising.com/blog/how-to-rank-higher-on-google-maps/

The same report flags something that applies directly to NEMO: **Google tells
service-area businesses to hide their address, and the industry has documented a
ranking penalty for the businesses that comply.** I am not recommending anyone
publish a home address — that is a real privacy decision and it is Eric's — but
it belongs in the record as a known cost of the SAB configuration this business
necessarily runs.

**2. The map-pack-vs-AI-citation gap is corroborated by two independent lines,
and one vendor summary contradicts it.** Yesterday I reported ~45% overlap
between map-pack winners and AI-recommended businesses from a PR firm's research
page and flagged it as unverified. Today: a June–July 2026 DerivateX study finds
**only 35% of AI Overview citations also rank in Google's top 10** for the same
query, and Ahrefs tracked that overlap **collapsing from 76% in July 2025 to 38%
by March 2026**. Separately, only ~12% of ChatGPT citations match Google page-one
URLs.
- https://www.marketingcode.com/ai-search-overview-local-pack-map-pack-contractor-citation-playbook-jul-2026/
- https://mshahid.com/blog/map-pack-vs-ai-search
- https://www.elev8operations.com/guides/ai-search-statistics-for-local-businesses-2026

**The contradiction, stated rather than resolved in my favour.** Secondary
coverage of the same Whitespark 2026 report says local and AI search signals
"have effectively merged" and that "the work that wins the Map Pack also wins
the AI answer". That is the opposite conclusion. I am siding with the overlap
studies because they are measurements of citation sets and the merge claim is a
vendor-blog gloss I could not trace to a primary — **but I am recording that a
credible source says my read is wrong**, and if it is, the GEO candidates in the
ledger are worth more than I priced them yesterday. This does not change today's
recommendations either way: both readings put the Business Profile first.

**3. Someone else's data reframes the pages-per-visit finding, and not in the
comforting direction.** Contractor-marketing sources make the point that a
single-page visit is often *not* a failure — "they clicked, they called, they
left" is a conversion, not a bounce. That is the benign reading of our 1.00.
**It does not survive contact with our own numbers: eleven sessions, eleven
single-page visits, and one call tap in twenty-five days.** People are arriving,
not going deeper, and not dialling. The benign explanation is the one thing the
data rules out.

The same sources give median contractor site conversion at **2–4% of visitors
into calls or forms**, with the top ~6% at 8–12%. Eleven beacon sessions and one
all-time tap is too small to compare against that, and I am not going to pretend
otherwise — but the *direction* is that even median performance would have
produced roughly zero calls from eleven visits. **At 2–3 humans a day, the site's
conversion rate is not currently a measurable quantity.** That is a volume
problem before it is a conversion problem, and it is an argument against
spending Eric's next hour on the website.
- https://instantbusinesspro.ai/post/contractor-website-getting-visitors-no-calls-conversion-fix
- https://pipelineon.com/blog/contractor-landing-page-tips/
- https://cufinder.io/blog/benchmarks/construction/

**4. Confirmed the GSC impression bug cannot explain our flood, with the exact
dates.** Google's logging error ran **2025-05-13 to 2026-04-27**. Our data
starts 2026-07-27, three months after the fix. Separately, Google's removal of
the `num=100` parameter in September 2025 cut bot-driven tool impressions by
40–60% industry-wide — which tells us rank-tracker impressions *were* a
documented mass phenomenon and that tools adapted after it.
...
- https://www.seo-kreativ.de/en/blog/gsc-impressions-bug-google-logging-error/

**Rejected today.**
- **A sticky mobile call bar.** Sources put it at a 25–40% lift in call volume
  and it was the single most-recommended conversion fix I found. **It is already
  shipped.** `growth/templates.py:160` emits it, `styles.css:229-268` styles it
  and switches it on at ≤680px, `analytics.js` counts its taps, and it is
  present on **40 of 41 HTML files** (the omission is `setup.html`, an admin
  page). This is exactly the FAQPage mistake the prompt warns about and it would
  have cost a slot in the list below.
- **Anything that adds pages.** Nine of eleven build steps noop'd this morning
  for lack of anything to build.
- **Any tactic aimed at impressions or coverage.** 26,144 impressions, 18 clicks.
- **Topping up the API credit as an urgent action.** Third episode, self-healed
  twice without intervention. Watch it, don't page anyone about it.

### Recommendations

**Nothing in this commit is live.** The site runs from
`/var/www/nemo-seamless-gutter`, which is not a git checkout, and
`publish_state.sh` copies droplet → repo only. Every on-site item below needs a
deploy before it does anything.

1. **Eric: twenty minutes on the Business Profile — primary category first, then
   the service-area list, then the itemised Services.** *(Free. T016/T051/T022,
   day 24.)* Ninth consecutive day at the top, and today it earned a number:
   **227 ranking weight on the primary category, 214 penalty for the wrong one**
   (research item 1). The category field is a bigger single lever than every
   on-site change this engine has made in twenty-four days combined. Screenshot
   first, one field at a time, do not pad the service areas. Skip the "Owner Q&A"
   half of T016 — the feature is gone. *How you would know:* the `york` bucket
   moves off 33/5/0, or `local_visitors` lifts off ~0, within a month. Judge on
   profile views and website clicks, not calls. *Checked:* T016, T051, T022 all
   `status: candidate, activated: null` in today's snapshot.
2. **Eric: reply to all 13 reviews in one sitting, then ask every finished job
   for one.** *(Free. T033 day 8, T007 day 24.)* Reviews are ~16% of pack weight
   with volume, recency and **response rate** as the three inputs, and research
   item 1 puts review signals among the biggest movers in the 2026 report.
   Neutral wording, everyone asked, good or bad — no incentives, no gating, no
   filtering. *How you would know:* review count and the date of the newest
   review, both visible on the profile. *Checked:* T033 and T007 both
   `candidate`/`activated: null`.
3. **Divine: deploy `growth/`.** *(Minutes. Day 12.)* Blocking nine things
   including both fixes from 08-17 and 08-18. Ship `scout.py` + `growth_daily.py`
   **as a pair** or the scout raises `TypeError`. *How you would know:*
   `coverage_pct` reads **39.5%** and `price.covered` reads **8** in the first
   snapshot after deploy.
4. **Eric: confirm the PA HIC registration number and put it on the site, the
   Business Profile and the estimate form.** *(Free if already registered.
   Day 5.)* PA law requires the registration number on advertising distributed in
   Pennsylvania and a website is advertising. This is legal exposure, not a
   growth idea, and it is the only item here that does not care whether the
   marketing works. **If Eric is not registered and does ≥$5,000/year, that is
   the finding and it outranks everything above.** *Checked:* fresh grep of all
   41 HTML files today; the only two hits for "licensed"/"insured" are the
   checklists quoted earlier.
5. **Someone must cap the tracked universe or change the headline. Day 12, and
   today it has an experiment behind it rather than a trend.** 4/4 scout-failure
   days froze the denominator; 10/11 scout-success days grew it by a mean of 6.2
   against an engine covering ~0.9/day. **Three options: (a) cap
   `tracked_queries` at a fixed universe and let the scout's finds queue outside
   it; (b) promote `top3 / ranked_known` — today 2 of 25 — to the headline;
   (c) rate-limit the scout to the engine's build rate.** I have not chosen,
   because this redefines the number Eric is judged against, and that is not my
   call. *(The engine could implement any of the three.)* I have moved this up
   from 7 to 5 because the evidence changed class today.
6. **Divine: delete the Schuylkill County section and rewrite the Akron lead.**
   *(Ten minutes. Days 10 and 22.)*
   `services/seamless-gutter-installation.html:227-230` and `:235` — remove the
   h2, the nine-town prose and the "Do you actually service Schuylkill County"
   FAQ. `services/gutter-guards.html:163` — "homes in Akron, PA and the
   surrounding Lancaster and York County area" → "homes across York County,
   Pennsylvania". **Leave line 199 alone.** Marked Divine because a commit here
   is reverted by the next publish. Both re-verified present today.
7. **Eric: make the Local Services Ads decision. T011, day 22.** *(Costs money —
   his call.)* Commit a capped monthly number before leaf-fall or reject T011 and
   stop carrying it. **A rejection is a real answer and would be the first entry
   ever in `does_not_work`.** *Checked:* T011 `candidate`/`activated: null`.
8. **Eric: pick three candidates and reject the rest.** *(Day 12. Pile 62,
   unchanged today only because the scout was dead. `does_not_work` empty after
   twenty-four days.)* Start by rejecting **T071** as already-built.

**One finding I am deliberately not making a new recommendation.** I grepped all
41 pages for testimonials, star ratings, `aggregateRating`, review counts or
customer quotes and found **none — the site carries zero customer proof of any
kind**, while the profile sits at 4.2 stars with 13 reviews. Research item 3
says social proof above the fold is one of the seven things that separate the
top 6% of contractor sites. But **the ledger already contains this fix twice** —
**T034** (`job_proof_pages`, one photographed page per finished install) and
**T043** (`thirty_second_phone_video_proof`) — both sitting as candidates since
before I started. Proposing it again as new would be exactly the padding I
criticised yesterday. *What is new* is the sharpest version of the credential
finding: `guides/best-gutter-company-york-county-pa.html:186` — the page
targeting our single most valuable money query — tells the reader that **"a short
list of direct questions will tell you more than any online review ever could."**
On a site with no reviews shown and no credentials stated, we are coaching
homeowners to distrust the one signal we have and to ask for the ones we never
answer. That is a copy problem on a generated page, so it needs the engine or a
deploy, not a repo edit that the next publish reverts.

### What I changed in this repo today

**Nothing.** Same reasoning as yesterday: there are twelve days of tested,
unshipped engine code in `main`, and a thirteenth commit would not make the site
faster, better or more findable. Today's verification work — reconstructing the
scout/denominator experiment from fifteen published snapshots, diffing the 40
flood rows, reading `gsc.py` for the window length, grepping the call-bar and
social-proof questions against the live files and `templates.py`, re-checking the
four outstanding free items — needed no commit and none of it is in the diff.

I touched no runtime state (`techniques.json`, `keywords.json`, `results.jsonl`,
`state.json`), activated, retired and re-statused nothing, and edited no page
copy or keyword list.

**A correction to my standing instructions, since the prompt asks for these.**
The prompt still describes the 07-28 baseline (77 rows, 17 matched, 3 clicks, 429
impressions, avg position 12.4; county bucket 2 of 50) and the 08-01 usage-limit
cap as current context. Today's figures are 935 / 25 / 18 / 26,144 / 25.0 with
the county bucket at 2 of 103, and the 08-01 cap expired eighteen days ago — the
live blocker is an empty credit balance, which is a different error with a
different fix. The prompt also says the watchdog mails the developer at 11:00 ET
if the engine looks dead; the engine failed three jobs this morning, so whether
that mail fired is a check worth someone making. **The data wins over the
prompt; the prompt should be updated.**

### Reasoning and uncertainties

Day twenty-four. Two top-3 queries county-wide, zero in all seven named towns,
zero AI visitors in twenty-five days, zero phone leads ever, one call tap seven
days ago, three bookings, ~2 human visitors a day, none of whom has ever viewed
a second page.

**The thing I would defend hardest today is the lead, and it is uncomfortable.**
For most of this journal I reported `coverage_pct` and `share_pct` as if they
described NEMO. This morning an unrelated billing failure ran the control
condition for me, for the fourth time, and the metric behaved exactly as the
denominator hypothesis predicts. I should have noticed after 08-12 and 08-13 —
the data was already in the repo and I had already published two entries that
quoted the falling percentage without checking why those two days were flat.

**Where I am least confident. Four things.**

*First, the experiment is small and not randomised.* Four treatment days, and
the scout's failures are not independent of anything else the API touches — the
same billing outage also stopped `strengthen_pages` and `money_pages` from
covering queries, so on those days the numerator was frozen too. The effect is
so lopsided (denominator +6.2/day vs numerator +0.9/day) that this cannot
reverse the conclusion, but it does mean the clean story is slightly dirtier
than my table makes it look.

*Second, I revised prediction 3 on a mechanism I inferred rather than observed.*
The rolling-window explanation for flat flood impressions fits `WINDOW_DAYS = 28`
and fits six days of near-zero movement, but I have not seen the flood decay or
fail to decay — 09-08 is still the test. I would rather be on record with the
revision now than quietly let the old date pass.

*Third, I may be wrong to keep declining the GEO candidates.* Research item 2
gives me two independent measurements supporting low map-pack/AI overlap and one
credible vendor summary saying the opposite. I resolved it in favour of my prior.
That is the shape of a mistake even when the answer is right, and **T046** — can
the AI crawlers even fetch this site? — is cheap enough that declining it twice
is starting to look like stubbornness rather than prioritisation.

*Fourth, still unchanged at three weeks: I cannot judge the generated copy at
scale.* Today I read one more generated page properly and found it arguing
against the value of online reviews on our most valuable query. One page is not
a sample. But it is the second time in two days that reading the actual copy
turned up something the metrics could never have shown me, and that ratio
suggests I should be reading pages instead of recomputing percentages.

**What would change my mind, dated.**
1. **2026-08-20:** if the credit failure repeats a second consecutive day, it
   stops being the self-healing weekly pattern of 08-05 and 08-12/13 and becomes
   something that needs Divine.
2. **On deploy:** `coverage_pct` reads **39.5%** and `price.covered` reads **8**.
   Anything else and the matcher behaves differently in production.
3. **2026-08-21:** the first honest 7-day beacon window closes. Medians 2–3
   confirms the reading; medians 6+ and I over-read a quiet week.
4. **2026-08-21:** the cadence commitment comes due. Recommendations 1, 2, 4 and
   6 are four free items — three of them twenty minutes of Eric's time and one a
   legal exposure — and all four are untouched. If they are still untouched on
   Friday, twenty-six days of daily review will have produced zero change in York
   County, and continuing daily is a choice to keep writing rather than to keep
   helping. **The review drops to weekly and three lines, HIC exempted.**
5. **2026-09-02:** if `coverage_pct` slides back toward ~33.5% after a deploy put
   it at 39.5%, the denominator diagnosis is confirmed in production and
   recommendation 5 stops being optional.
6. **2026-09-08:** the flood-decay test. I now predict the 40 rows are **still
   there**; if they have gone, my rolling-window model is wrong and the
   one-off-burst reading was right.
7. **On the profile:** if Eric does 1 and 2 and the `york` bucket is still 33/5/0
   with zero new calls a month later, the map pack is not the lever either, and
   the honest question becomes whether a one-truck operator can win this county
   on search at all.
...
## 2026-08-20 — review agent

### Lead: the credit failure repeated, the tripwire I set yesterday fired — and I have to immediately weaken my own tripwire, because my own table already showed a two-day episode

Yesterday's entry ended with a dated commitment: *"2026-08-20: if the credit
failure repeats a second consecutive day, it stops being the self-healing weekly
pattern of 08-05 and 08-12/13 and becomes something that needs Divine."*

It repeated. This morning's 06:00 run failed **four** jobs, up from three:

- `geo_answer_first_content_pass` — FAILED (new today), `/areas/seamless-gutters-spring-grove-pa.html`
- `strengthen_pages` — FAILED, 3 candidates
- `money_pages` — FAILED, 'instalacion de canaletas york pa'
- **scout** — did not run

All four with `"Your credit balance is too low to access the Anthropic API"`.
`last_build.new` **0**, `last_build.changed` **0**. Seven of eleven steps
no-op'd. Second consecutive day the engine produced nothing.

**Now the part I have to say against myself.** The tripwire fired by its letter,
but by the evidence in my own table it should not have been set at two days.
That table records **08-12 and 08-13 as consecutive failures** that self-healed
on 08-14. So "second consecutive day" is not a break in the pattern — it is an
exact repeat of the previous episode's length. **08-21 is the day that actually
breaks it.** I set the threshold one day too tight yesterday and I am correcting
it rather than letting a fired tripwire manufacture an urgency the data does not
support.

What *is* genuinely new is the **spread**: the failure reached
`geo_answer_first_content_pass`, a third content technique, which was clean
through both prior episodes. Three techniques plus the scout is a wider outage
than 08-05 or 08-12/13.

I have notified the owner, framed exactly this way — fired, but weaker than
yesterday's wording implied — because the fix is minutes and small money and
nobody is reading this file today.

**And one thing nobody should miss about the economics of this outage.** The
scout is the denominator engine. Four days of scout failure produced four days
of frozen denominator (see below, now 5/5). So **restoring the credit will
resume denominator inflation and push `coverage_pct` back down**, and that fall
will look like the business getting worse. Restoring credit and capping the
tracked universe are the same decision and should be made together, or the next
two weeks of this journal will report a decline that is an accounting artefact.

### Where the numbers stand

**Goal metric: `top3` = 2 of 195, `share_pct` 1.0% against a 50% target.
Flat at 2 for the eleventh consecutive day.** `top10` 7, `ranked_known` 25 —
both flat. **Zero top-3 positions in all seven named towns, day twenty-five:**
york 33 tracked / 5 covered / **0** top-3, dover 16/7/0, red-lion 11/3/0,
dallastown 11/4/0, spring-grove 11/3/0, hanover 10/4/0. Both top-3s are in the
county bucket, 2 of 103.

**`coverage_pct` 32.3% and `tracked_queries` 195 — flat for a second
consecutive day**, because the scout is dead. The denominator experiment is now
**5 scout-failure days out of 5 with zero denominator movement**, against 10 of
11 scout-success days growing it by a mean of +6.2/day.

**Search Console: 964 rows / 25 matched / 18 clicks / 26,613 impressions /
avg position 25.1.** Against yesterday (935 / 25 / 18 / 26,144 / 25.0): rows
**+29**, matched **0**, clicks **0**, impressions **+469**, avg_position **+0.1
(worse)**. Yesterday's one bright number — `matched` moving 23 → 25 — did not
continue. **469 new impressions bought zero clicks.**

**The out-of-area flood is unchanged and is now most of our impressions.**
`discovered_untracked` holds **40 rows, all 40 outside York County**, totalling
**12,066 impressions and 0 clicks** — that is **45% of the site's entire
impression count from queries that cannot ring this phone.**

**Coverage by intent, unchanged:** hire 55/118, check 4/22, diy 1/8,
**price 3 of 47 (6%)**. See research item 4 and recommendation 7 — the price
lane is not only uncovered, the pages we already have on it are broken.

**Traffic, sixth beacon day: 3 visitors, 6 pageviews** (2 organic, 1 direct).
Beacon days 08-14 → 08-19: visitors **2, 2, 2, 3, 2, 3**; pageviews
**2, 2, 2, 3, 2, 6**.

**Leads. `bookings_all_time` 3, `phone_leads_all_time` 0.** `call_taps` still
**1 all-time** (08-12) — **eight consecutive days with no tap**. **`ai_calls` 0
since the two on 07-30 — twenty consecutive days in which not one non-owner
number rang the phone assistant.** `ai_visitors` 0 across twenty-six days.

**Ledger: 74 techniques — 11 active, 1 retired, 62 candidates. `does_not_work`
is still `[]` on day twenty-five.** Unchanged today only because the scout that
generates candidates was dead.

**Deploy: day 13.** `code_version`, `log_visitors`, `keywords.ranked` all still
absent. New tell found today, and it costs a specific measurement — see
"Did previous changes work?", item 8.

### Did previous changes work?

**1 — Deploy lands `coverage_pct` at 39.5% and `price.covered` at 8. Still
untestable, day 13.** `price.covered` reads **3**. Not re-deriving the estimate.

**2 — The 7-day beacon window closes 08-21. Six of seven days in: 2, 2, 2, 3,
2, 3.** Median 2. One day left and it cannot overturn the ~2–3-humans-a-day
reading.

**3 — Pages-per-visit is stuck at 1.00, so the series carries no information.
This broke yesterday, and I am flagging it because I said I would watch it.**
08-19 was **3 visitors and 6 pageviews — 2.00 pages per visit**, the first
multi-page day of the beacon era, after five straight days at exactly 1.00.
**Verdict: cannot be told apart from noise, and it is not close.** Three
visitors. One person opening two pages moves this number from 1.00 to ~1.7 on
its own. What it does do is retire the stronger claim — "not one person has ever
viewed a second page" was true for five days and is no longer true. Seventeen
pageviews across fourteen sessions is still not a sample.

**4 — `log_visitors` ~12 vs beacon ~2. Blocked on the deploy, day 13.**

**5 — The flood does not age out; the 40 rows persist until the poller stops.**
Holding. Today: still exactly **40 rows**, still **all 40 out-of-area**, still
**0 clicks**, and the bellwether row "gutter installer" moved **472 → 470
impressions** — a two-impression *decrease* on a 28-day rolling window, which is
noise around flat, not decay. The 09-08 test stands and I still predict the rows
are there. The second-burst tripwire (rows +50 on three of five days) did not
fire on +29 today, but +29 is the largest single-day row gain in a week and is
worth one more day's attention.

**6 — The profile lever. Untestable, day 25. Recommendations 1 and 2 from every
prior entry have not been done.**

**7 — Yesterday's credit tripwire. Fired, and partially self-refuted.** Covered
in the lead.

**8 — A prediction I did not make, which the deploy has now cost us.** I went
looking for whether AI answer-engine crawlers are actually fetching this site —
T046's question, which two prior entries called cheap and declined. The repo has
the answer half-built and undeployed. `growth/metrics.py:97-108` defines nine
per-crawler counters (`crawl_gptbot`, `crawl_oai_searchbot`, `crawl_claudebot`,
`crawl_perplexitybot`, `crawl_applebot`, …) **with `bingbot` and `googlebot`
deliberately included as controls** — the comment at `metrics.py:94-96` says so
explicitly: *"if it is fetching and GPTBot is not, the block is AI-specific
rather than the site being down."* `growth/snapshot.py:51` publishes them.
**Not one `crawl_*` series is in today's snapshot.** So the measurement that
answers T046 exists, is written, is tested, and is sitting in `main` unshipped.
**T046 is not cheap-and-declined; it is done-and-undeployed.** That is a
concrete cost of day 13 that no prior entry priced.

**9 — A correction to my own reading, caught before I published it.** I nearly
reported that `ai_calls` counts answer-engine crawler fetches, and that two
fetches in twenty-two days meant the crawlers were not coming. That is wrong.
`growth/calls.py:1-13` is explicit: **`ai_calls` is inbound phone calls that
reached the ElevenLabs phone assistant**, owner and 555 numbers excluded,
outbound excluded. So the 07-30 pair were **two real people ringing the
business**, and the twenty-day zero since is a demand signal, not a crawl
signal. The crawl question remains genuinely unmeasured from here — see item 8.

**Not actioned, with day counts.** `growth/` deploy **day 13**; GBP category /
service-area / services **day 25**; review replies **day 9** (T033) / **day 25**
(T007); PA HIC registration **day 6**; the Schuylkill passages **day 11**; the
Akron paragraph **day 23**; the LSA decision **day 23**; the goal-metric
definition **day 13**; candidate triage **day 13**.

### What I researched today

**1. There is a hard seasonal clock on every recommendation in this journal, and
it started roughly now.** Trade sources are consistent: start fall gutter
campaigns **6–8 weeks ahead of peak leaf drop**, because homeowners call the
first time it rains hard after the leaves come down, and anyone marketing in
October is bidding against a booked-out calendar. Peak leaf drop in south-central
Pennsylvania runs mid-to-late October, which puts the window at **now through
roughly mid-September**. One source states it plainly: companies that start
optimising in November are already too late, and local SEO is built in the
off-season so the ranking is ready when demand arrives. This does not add a new
recommendation — it puts a deadline on the ones that have been sitting for
twenty-five days, and it is the reason I am not letting the credit outage take
the top of the list on its own merits.
- https://www.roofingcontractor.com/blogs/16-roofing-contractor-blog/post/96159-getting-the-most-out-of-gutter-leads-this-fall
- https://www.elev8operations.com/guides/how-to-get-more-gutter-leads-2026
- https://omnyra.ai/blog/gutter-company-local-seo

**2. Freshness is now a first-class local-pack signal, and it is a different
lever from the category audit I have recommended nine times.** 2026 sources
report that Google's local ranking has shifted toward present-tense signals —
what you posted this week, what photos went up yesterday, which reviews you
answered this morning — and that **profiles with no updates in 30+ days can see
impression declines**. Quantified: **photos drive ~42% more direction requests
and ~35% more website clicks**; posts at **twice a month** correlate with ~13%
higher branded engagement, with 2026 guidance pushing toward **twice a week**.
GBP signals remain ~32% of local pack weight.
- https://nullstacks.com/blog/google-business-profile-freshness-sentiment-audit-2026/
- https://biziq.com/blog/google-business-profile-optimization-statistics/
- https://www.theadfirm.net/dynamic-profiles-are-the-new-local-ranking-factor-heres-how-to-win/

This matters for NEMO specifically because a one-truck operator who is already
on roofs every day is uniquely well-placed to satisfy a freshness signal at zero
cost — it is a phone camera and ninety seconds, not an agency retainer. It also
corroborates yesterday's note that Q&A is gone: **"Ask Maps" (Gemini-powered)
replaced it in early 2026**, which is why T016's Q&A half should stay skipped.
- https://www.agencyjet.com/blog/google-business-profile-optimization-guide/

**3. The Local Services Ads decision has acquired an expiry date, and the terms
changed underneath it.** Google is folding LSAs into Google Ads as pay-per-lead
Performance Max campaigns. **Phase one began this month (August 2026)** for
selected US storefront and home-service categories — the named list is plumbing,
HVAC, electrical, appliance repair, house cleaning, lawn care, roofing, pest
control, moving; **gutters is not in it**. **Service-area businesses without a
storefront — which is exactly NEMO — migrate later in 2026.** After migration
the standalone LSA dashboard is retired and **manual max cost-per-lead bidding
is gone**. Reported average LSA cost per lead is around **$53**.
...
- https://locafy.com/blog/local-services-ads-migration-google-ads
- https://www.cornerstonead.com/2026/07/29/google-local-services-ads-update/

**This makes T011 worse than I have been pricing it, not better.** Starting LSAs
in September means Eric learns a dashboard that is scheduled to be taken away
from his business class within months, at ~$53 a lead, in the one season where
his time is scarcest. I am not rejecting it for him, but the honest version of
the recommendation has changed from "decide" to "decide, knowing the product is
mid-migration."

**4. Pricing transparency, and the reason it is not a generic recommendation
today.** 2026 contractor-marketing sources argue that hiding price loses buyers
before contact and that homeowners want a rough number before they will talk.
I normally discard this as evergreen advice. I am keeping it because it named a
defect I then found in our own copy — see recommendation 7.
- https://hookagency.com/blog/homeowners-looking-for-contractors/
- https://gohighpointdigital.com/guide/what-every-contractor-website-needs-to-convert-visitors-into-calls/

**5. Where AI answers about local contractors actually come from — this
re-prices two candidates already in the ledger.** **Over 70% of local business
results in ChatGPT reportedly come from Foursquare**, with Apple Maps, Yelp and
GBP as the other structured sources; a 2026 local-service audit found
**BuildZoom cited for 100% of trades tested**, used to verify licence class and
permit history. AI search reportedly surfaces only ~1.2% of local businesses,
and only ~45% of map-pack winners appear in AI answers.
- https://www.marketingcode.com/1-2-percent-local-businesses-ai-search-trust-stack-may-2026/
- https://cited.so/blog/ai-seo-for-local-business
- https://evolveamz.com/local-business-ai-search-guide/

**I am not proposing this as new — the ledger already has it twice.** **T048
`foursquare_places_claim_for_ai_answers`** and **T053
`buildzoom_trade_verification_profiles`** are both candidates and have been
since before I started; **T029 `apple_bing_place_cards`** covers Apple Maps.
What is new is that they should not be ranked as speculative GEO experiments.
If AI engines answer local trade queries out of places databases rather than out
of contractor websites, then a free one-time claim of a Foursquare record is a
better-evidenced AI play than any on-site content this engine can generate — and
it explains twenty-six days of `ai_visitors` 0 without needing a crawler theory.

**Rejected today.**
- **Adding pricing content as a new technique.** `money_pages` (T002) already
  exists and `growth/techniques.py:379-390` already instructs it to publish
  ranges. The problem is a contradiction in the code, not a missing feature —
  recommendation 7.
- **Proposing Foursquare / BuildZoom / Apple Maps as new ideas.** T048, T053 and
  T029, all already candidates. Re-ranked, not re-proposed.
- **An AI-crawler access audit as a new task.** T046, and the measurement is
  already written and undeployed — `metrics.py:97-108`, `snapshot.py:51`.
- **A sticky mobile call bar.** Already shipped (`templates.py:160`,
  `styles.css:229-268`); verified again by a prior entry.
- **Anything that adds pages.** Seven of eleven build steps no-op'd for lack of
  work, and the three that had work failed on billing.
- **Any tactic aimed at impressions.** 26,613 impressions, 18 clicks, and 45% of
  those impressions are out-of-county rank-tracker noise.

### Recommendations

**Nothing in this commit is live.** The site runs from
`/var/www/nemo-seamless-gutter`, which is not a git checkout, and
`growth/publish_state.sh` copies droplet → repo only. Every on-site item below
needs a deploy before it changes anything a homeowner can see.

**Items 2, 3 and 4 are free, are Eric's, and are the ones with the leaf-fall
clock on them.** If only one thing happens this week, it should be one of those,
not item 1.

1. **Divine: top up the Anthropic credit — and make the denominator decision at
   the same time, or don't bother.** *(Minutes, small money. Second consecutive
   dead day.)* Restoring credit alone restarts the scout, which adds ~6.2
   queries/day to a denominator the engine covers at ~0.9/day, which drives
   `coverage_pct` and `share_pct` down and makes the next fortnight's journal
   read like decline. Three options, unchanged and still not mine to pick:
   **(a)** cap `tracked_queries` at a fixed universe and queue the scout's finds
   outside it; **(b)** promote `top3 / ranked_known` — today **2 of 25** — to the
   headline; **(c)** rate-limit the scout to the engine's build rate.
   *How you would know:* `last_build.new`/`changed` go non-zero and
   `last_scout.ok` is true. *Checked:* `snapshot.json` → `last_build.log`
   (4 failures) and `last_scout.detail`. *(The engine could implement any of the
   three caps itself; only the top-up needs a human.)*

2. **Eric: twenty minutes on the Business Profile — primary category first, then
   the service-area list, then the itemised Services.** *(Free. T016/T051/T022,
   day 25.)* Tenth consecutive day at or near the top of this list. The 2026
   evidence is that the primary category is the single heaviest local-pack
   factor and a wrong one carries an active penalty; GBP signals are ~32% of pack
   weight. New today: **list each legitimate town as its own service-area entry
   rather than one broad polygon — granular entries rank better on the specific
   town queries**, which is precisely the shape of our failure (0 top-3 across
   all seven named towns). Do not pad with towns Eric will not drive to. Skip the
   Q&A half of T016 — the feature is gone, replaced by Ask Maps. *How you would
   know:* the `york` bucket moves off 33/5/0, or `local_visitors` lifts off ~0,
   within a month; judge on profile views and website clicks, not calls.
   *Checked:* T016, T051, T022 all `status: candidate`, `activated: null` in
   today's `snapshot.json`.

3. **Eric: one job photo and one short update to the profile per week, starting
   this week. This is new to the top of the list and it is the seasonal one.**
   *(Free. T008 `gbp_posts` and the photo half of T022 — both candidates.)*
   Research item 2: freshness is now a present-tense ranking input, profiles idle
   30+ days can lose impressions, and photos correlate with ~42% more direction
   requests and ~35% more website clicks. Research item 1: the window to be
   ranking for leaf-fall closes around mid-September. Eric is on a roof most
   days; this is a phone camera and ninety seconds, not a project. **No incentives,
   no stock images, no invented claims — actual jobs, actual town names.**
   *How you would know:* profile views and website clicks in the GBP dashboard
   over four weeks; secondarily `local_visitors`, which has produced 3 in
   twenty-six days. *Checked:* T008 and T022 both `candidate`/`activated: null`;
   and the engine cannot do this — nothing in `growth/techniques.py` touches the
   Business Profile, every active technique writes files into the site.

4. **Eric: reply to all 13 reviews in one sitting, then ask every finished job
   for one.** *(Free. T033 day 9, T007 day 25.)* Review signals are ~16–20% of
   pack weight with volume, recency and response rate as inputs, and response
   rate is the half that costs nothing and is entirely within his control.
   Neutral wording, everyone asked, good or bad — **no incentives, no gating, no
   filtering.** *How you would know:* review count and newest-review date on the
   profile. *Checked:* T033 and T007 both `candidate`/`activated: null`.

5. **Divine: deploy `growth/`. Day 13, and it now has a cost I can name.**
   *(Minutes.)* Beyond the coverage and price-queue fixes, the deploy carries the
   **nine per-crawler counters that answer T046** — `metrics.py:97-108` with
   Bingbot and Googlebot as deliberate controls, published by `snapshot.py:51`,
   and **absent from every snapshot to date**. Until it ships, "do answer engines
   fetch this site at all?" is unanswerable from the repo, and `ai_visitors` 0
   for twenty-six days has no diagnosis. Ship `scout.py` + `growth_daily.py`
   **as a pair** or the scout raises `TypeError`. *How you would know:*
   `coverage_pct` reads **39.5%**, `price.covered` reads **8**, and `crawl_*`
   series appear in the first snapshot after deploy.

6. **Eric: confirm the PA HIC registration number and put it on the site, the
   Business Profile and the estimate form.** *(Free if already registered. Day
   6.)* PA law requires the registration number on advertising distributed in
   Pennsylvania, and a website is advertising. This is legal exposure, not a
   growth idea, and it is the only item here that does not care whether the
   marketing works. **If Eric is not registered and does ≥$5,000/year, that is
   the finding and it outranks everything above.** *Checked:* a prior entry's
   grep of all 41 HTML files found only two hits for "licensed"/"insured", both
   checklists telling the reader to demand credentials NEMO never states about
   itself — `services/seamless-gutter-installation.html:243` and
   `guides/gutter-services-near-me.html:181`.

7. **The engine contradicts itself about price, and it has broken our
   cost pages. Eric decides the policy; the engine implements it; it needs a
   deploy.** *(New today.)* `growth/techniques.py:379-390` (`MONEY_SYSTEM`) tells
   the writer that cost queries are the most valuable traffic a contractor can
   get, that refusing to discuss price is *why* national lead-gen sites outrank
   contractors, and that it must publish **"honest ranges for south-central
   Pennsylvania."** But `growth/techniques.py:1580-1583` — the rules governing
   the rewriters — carry the hard rule **"Do NOT invent prices, dollar figures,
   ranges,"** and `techniques.py:256-257` (`AREA_SYSTEM`) says **"never quote a
   number."** The result is visible in the published site:
   - `guides/how-much-do-seamless-gutters-cost.html` **has** figures ($7 and $14
     per foot, $1,500–$4,000).
   - `guides/gutter-cleaning-cost-york-pa.html` — the page targeting the fall
     cleaning price query, in the season those searches spike — **has a section
     headed "Typical price ranges" that promises "general ballpark figures…
     With that caveat:" and then contains not one number.** Its three bullets
     say "a modest flat fee", "cost more", "add to the base figure", and its FAQ
     answer to *"How much does gutter cleaning cost in York, PA?"* lists factors
     and never answers. A homeowner reads a heading that promises a range,
     receives none, and is then told "we won't promise a specific guaranteed
     price sight-unseen."

   That is a page breaking a promise it makes to the reader three lines earlier,
   on the intent bucket that is **3 of 47 covered**. *The decision Eric owns:*
   whether this business publishes ballpark ranges at all. *If yes,* the engine
   change is to make the no-figures rule intent-aware so cost pages follow
   `MONEY_SYSTEM`, and to force one regeneration of the affected pages — the
   rewriter cannot fix this on its own, because the rewriter is the thing
   forbidding numbers. *If no,* the fix is smaller and still required: delete the
   "Typical price ranges" heading and its caveat sentence, because a section that
   announces figures and withholds them is worse than no section. **I did not
   make either edit — see the note below.** *How you would know:* clicks on the
   two cost pages in GSC, and `price.covered` after the deploy.

8. **Eric: kill the pile. Reject fifty-nine of the sixty-two candidates and keep
   three.** *(Day 13. `does_not_work` empty after twenty-five days.)* Twenty-five
   days of review has produced a growing list and zero rejections, which is my
   failure as much as anyone's. So here is the shortlist rather than the pile —
   **T048 (Foursquare places claim), T053 (BuildZoom trade verification), T029
   (Apple/Bing place cards)**: three free, one-time, off-site record claims which
   research item 5 says are where AI answers about local contractors actually
   come from, and which cost an evening in total. Start the rejections with
   **T071** (already built) and **T011** — see below. *Checked:* all four IDs
   read `candidate`/`activated: null` in today's snapshot.

   **On T011 (Local Services Ads), day 23, the terms changed this month.**
   Research item 3: LSAs are migrating into Google Ads as pay-per-lead
   Performance Max; NEMO's class — service-area business, no storefront —
   migrates later in 2026; the standalone dashboard goes away and manual
   cost-per-lead bidding with it; typical CPL ~$53. **Entering now means learning
   a dashboard scheduled to be retired for his business type within months,
   during his busiest season.** I still will not spend Eric's money for him, but
   my recommendation is now explicitly to **reject T011 for this leaf season** and
   revisit after the migration settles. A rejection would be the first entry ever
   in `does_not_work`, and that scoreboard being empty for twenty-five days is
   itself a finding.

### What I changed in this repo today

**Nothing.** Thirteen days of tested, unshipped engine code sit in `main`; one
more commit does not make the site faster, better or more findable, and a
prompt change that causes a generator to start emitting dollar figures on a
contractor's website is a business decision about what NEMO is willing to quote,
not a bug fix I am entitled to make unilaterally at 11am while nobody is
reading. I considered making the recommendation-7 edit and declined for that
reason and because it would land untested at the back of a thirteen-day deploy
queue, where it would raise the risk of the deploy that matters more.

I touched no runtime state (`techniques.json`, `keywords.json`, `results.jsonl`,
`state.json`), activated, retired or re-statused nothing, and edited no page copy
or keyword list.

**Corrections to my standing instructions.** The prompt still describes the
07-28 baseline (77 rows, 17 matched, 3 clicks, 429 impressions, avg position
12.4; county bucket 2 of 50) and the 08-01 usage-limit cap as current. Today:
**964 / 25 / 18 / 26,613 / 25.1**, county bucket **2 of 103**, and the 08-01 cap
expired nineteen days ago — the live blocker is an **empty credit balance**,
a different error with a different fix. The prompt also warns to suspect the
measurement if `visitors` sits at zero for a week; that guidance is now stale in
a specific way — since the 08-14 beacon cutover the honest count is ~2–3/day,
not zero, and the thing to suspect is a **beacon that stops firing**, which
`log_visitors` is designed to catch and which is undeployed. **The data wins over
the prompt; the prompt should be updated.**

### Reasoning and uncertainties

Day twenty-five. Two top-3 queries county-wide, zero in all seven named towns,
zero AI visitors in twenty-six days, **zero calls to the phone assistant in
twenty days**, one call tap eight days ago, three bookings all time, ~2–3 human
visitors a day, and 45% of our search impressions coming from Philadelphia-ring
towns that will never call.

**The thing I would defend hardest today is that I weakened my own tripwire.**
I set it yesterday at "second consecutive credit failure means escalate", and my
own table on the same page recorded a previous two-day episode that self-healed.
Firing an alarm on a threshold my own evidence contradicts would have taught
Eric to ignore the next one, which is a more expensive mistake than a day of
dead automation. I notified him anyway, with the caveat attached, because the
fix is small and the alternative is silence on a second dead day.

**The thing I got wrong and caught.** I spent twenty minutes convinced that
`ai_calls` measured crawler fetches and that the site was invisible to answer
engines. `growth/calls.py:1-13` says otherwise in plain English. I would have
published a confident, wrong, load-bearing finding. It is the second day running
that reading the actual source beat reasoning from the metric name, and the
lesson generalises: this snapshot's field names are not self-documenting.

**Where I am least confident. Four things.**

*First, the seasonal clock is the weakest-sourced claim I am leaning on hardest.*
"6–8 weeks before peak leaf drop" comes from trade blogs, not from a measurement,
and I have no York County search-volume curve to check it against — GSC gives me
twenty-six days, which cannot show a season. I believe it because it is
mechanically sensible (people call after the first hard rain post-leaf-drop) and
because every source agrees, not because I verified it. If it is wrong, item 3
drops down the list and nothing else changes.

*Second, I may be over-reading the price defect.* I found it on one page. It is a
real internal contradiction and I can quote the code that causes it, but I have
read maybe five of the forty-one generated pages properly. There may be nine more
defects of this shape, or this may be the only one. **What I actually believe is
that nobody has audited the generated copy at all, and that the ratio of
findings-per-page-read is far higher than findings-per-percentage-recomputed.**

*Third, the Foursquare number is a single-source statistic doing a lot of work.*
"Over 70% of ChatGPT local results come from Foursquare" appears in vendor
marketing content and I could not trace it to a primary methodology. I am using
it to re-rank existing candidates, not to justify spending, which is about as
much weight as it can carry. If it is inflated, T048 is still a free one-time
claim and the downside is an evening.

*Fourth, I have now written twenty-five daily reviews and York County has not
changed.* The recommendations are not getting better; they are getting older.
Recommendation 2 has appeared ten times.

**What would change my mind, dated.**
1. **2026-08-21:** if the credit failure runs a **third** consecutive day, it
   genuinely breaks the 08-05 and 08-12/13 pattern and needs Divine, not a note.
2. **2026-08-21:** the seven-day beacon window closes. Medians 2–3 confirms
   ~2–3 humans a day; medians 6+ and I over-read a quiet week.
3. **2026-08-21:** the cadence commitment comes due, and I am keeping it.
   Recommendations 2, 3, 4 and 6 are free and Eric's; three cost twenty minutes
   and one is legal exposure. **If all four are still untouched tomorrow, this
   review drops to weekly and three lines, HIC exempted** — twenty-six days of
   daily writing that produced no change in York County is a habit, not a
   service.
4. **On deploy:** `coverage_pct` **39.5%**, `price.covered` **8**, and `crawl_*`
   series present. If `crawl_gptbot` is 0 while `crawl_googlebot` is non-zero,
   the AI-invisibility problem is access, and T046 becomes urgent. If both are
   non-zero, the crawlers come and are not citing us, and recommendation 8's
   places-database reading is the right one.
5. **2026-09-08:** the flood-decay test. I predict the 40 rows are still there.
6. **2026-09-15:** the seasonal window closes. If the Business Profile is
   untouched by then, leaf season 2026 is decided and the honest thing to write
   is that this channel did not get its chance rather than that it did not work.
7. **On the profile:** if Eric does 2, 3 and 4 and the `york` bucket is still
...
## 2026-08-21 — review agent

### Lead: the engine ran this morning, published no state, and told nobody. I am reviewing yesterday's numbers.

There is no `2026-08-21 — engine run` entry above this one, and there is no new
`growth/snapshot.json`. Today's publish commit — `ccc0fb6`, "Publish growth
state for 2026-08-21" — **changed one file, `sitemap.xml`, by one line.** Every
other daily publish in this repo's history carried `growth/snapshot.json` and
`growth/JOURNAL.md`. I checked all ten of the most recent ones; they all do.

So the state file I am reviewing is dated **2026-08-20** and its traffic series
ends **2026-08-19**. Every number in this entry was already reported yesterday.
I have no new data at all.

**What actually happened, as far as the repo can tell me.** The one line that
did change is the `lastmod` on `/reports/waiting-on-you.html`, 08-19 → 08-20.
That file is written by the `report` step and its date is picked up by
`rebuild_sitemap` on the *following* run. So this morning's run got at least as
far as `rebuild_sitemap` — **the engine is alive and the cron fired.** What
failed is narrower: `snapshot.write()` raised.

`growth_daily.py:220-228` wraps that call in a try/except that logs
`snapshot: FAILED — {e}` and continues. That is the right design — a snapshot
bug should not stop the site build — but it means the only record of the reason
is one line in `/var/log/nemo-growth.log`, which I cannot read. The exception
came before `append_journal`, which is why the engine's own entry is missing too.

**Why nobody was told, which is the part worth fixing.** `cmd_watchdog`
(`growth_daily.py:237`) checks three things: whether `last_build` has gone
stale, whether today's build had failing steps, and whether the GSC sync is
erroring. All three read state the engine *wrote*. **None of them can see a
write that did not happen.** The engine is healthy everywhere except the single
file the cloud agent depends on, so the 11:00 watchdog will look at it, find a
completed build, and stay quiet about this. Eric will get mail today — the
credit failures still trip the failed-steps check — but nothing in it will say
the snapshot is missing.

**I fixed that.** See "What I changed in this repo today". It needs a deploy.

**What this costs today.** Three dated commitments from yesterday came due this
morning and the feed went dark on the same morning. Two of the three cannot be
evaluated at all — details below. I am not going to guess at them.

### Where the numbers stand

**All of this is 2026-08-20 data, republished, not re-measured.** I am
restating it only so this entry stands on its own; nothing here moved, because
nothing here was re-read.

**Goal metric: `top3` = 2 of 195 tracked queries, `share_pct` 1.0% against a
50% target.** Flat at 2 for the twelfth consecutive day. `top10` 7,
`ranked_known` 25 — against `ranked_known` the honest reading is **2 of 25**.
**Zero top-3 positions in all seven named towns:** york 33 tracked / 5 covered /
**0** top-3, dover 16/7/0, red-lion 11/3/0, dallastown 11/4/0, spring-grove
11/3/0, hanover 10/4/0. Both top-3s sit in the county bucket, **2 of 103**.

**Direction: unchanged.** I cannot say "it moved" or "it held" for 08-21,
because 08-21 was never measured.

Coverage 32.3%, `tracked_queries` 195. By intent: hire 55/118, check 4/22,
diy 1/8, **price 3 of 47**. Search Console at last reading: 964 rows, 25
matched, 18 clicks, 26,613 impressions, avg position 25.1.

**Traffic, beacon days 08-14 → 08-19: 2, 2, 2, 3, 2, 3 visitors.** Median 2.
**Leads: `bookings_all_time` 3, `phone_leads_all_time` 0**, one call tap ever
(08-12), `ai_calls` 0 since 07-30, `ai_visitors` 0 across the whole series.

**Ledger: 74 techniques — 11 active, 62 candidates, 1 retired.
`does_not_work` is still empty on day twenty-six.**

### Did previous changes work?

**1 — "If the credit failure runs a third consecutive day it needs Divine."
Cannot be evaluated, and it is not a close call.** The file that would record
today's build was never written. As of 08-20 it was day two, matching the
08-12/13 episode exactly. Today is the day that would have broken the pattern
and I have no way to see it. **I have escalated to Divine anyway** — not on the
credit tripwire, which remains unresolved, but on the snapshot failure, which
is a new and independent problem. The credit question rides along in the same
message because it costs nothing to ask both at once.

**2 — "The seven-day beacon window closes 08-21." Six of seven days in, and
the seventh is missing.** 08-14 → 08-19: **2, 2, 2, 3, 2, 3**. Median 2, mean
2.33, range 2–3. **Verdict: I am closing this at six days rather than leaving
it open indefinitely.** Six consecutive days inside a two-visitor band is
already a tighter result than the seven-day test was designed to produce; a
seventh point cannot move a median of 2 to anything near the 6+ that would have
falsified it. **The site gets about two to three human visitors a day.** That
is the honest number and I am going to stop re-testing it.

**3 — "If recommendations 2, 3, 4 and 6 are still untouched, this review drops
to weekly and three lines." Cannot be evaluated, and I am not executing it
today.** Those four are Business Profile and HIC actions. Their signals —
`local_visitors`, the `york` bucket, profile views — live in the snapshot that
was not written, and GBP actions were never visible to me directly in any case.
Firing a self-imposed demotion on absent evidence would be the same error I
criticised myself for on 08-20, in the other direction.

**But I do not want to leave it as "deferred" and quietly let it lapse, because
that is how commitments die.** So I am revising it on its merits instead, and
saying why. The premise was that twenty-six days of daily review produced no
change in York County. That premise is still true. What today shows is that the
premise was incomplete: **this review caught an engine failure that no other
part of the system was watching for, on the one morning it happened.** The
watchdog could not see it, the engine could not report it, and by tomorrow the
evidence — a one-file publish commit — would have been buried under a normal
one. That is real value, and it is a different kind of value from the strategy
essay.

So the honest revision is a split, not a demotion:
- **Daily, short:** an engine-health check. Did the run complete, did it publish,
  what failed, what does the goal metric read. Ten lines. This is the part that
  earns its keep.
- **Weekly, long:** the research-and-recommendations essay. Recommendation 2 has
  now appeared eleven times. Writing it a twelfth time tomorrow will not make
  Eric do it, and the repetition is actively devaluing the entries around it.

**New commitment, dated: the first review after a fresh snapshot arrives runs in
the short form, and the long form runs on Mondays.** HIC stays exempt from any
cadence change, as before.

**4 — "The flood does not age out; the 40 rows persist." Holding, untested
today.** Still 40 rows at last reading. The 2026-09-08 test stands. But I found
something today that weakens my own explanation of it — see research item 1.

**5 — "Deploy lands `coverage_pct` at 39.5% and `price.covered` at 8." Still
untestable, day 14.** What I *can* now report, which no prior entry checked:
**I ran the test suite in this repo and it passes — 212 tests, and 218 after
mine.** The fourteen-day deploy queue is not a pile of unverified work; it is
green. That removes one of the stated reasons for hesitating over it.

**6 — The profile lever. Untestable, day 26.** Recommendations 2, 3 and 4 have
not been actioned as far as any measurement available to me can tell.

**7 — "There may be nine more price defects of this shape, or this may be the
only one." Tested today. It is not the only one — there are four.** Yesterday's
entry found `guides/gutter-cleaning-cost-york-pa.html` promising "general
ballpark figures … With that caveat:" and then delivering none. I grepped every
one of the 41 pages for dollar figures and cross-referenced against price
headings. **Exactly one page on this site answers a cost question with a
number** — `guides/how-much-do-seamless-gutters-cost.html` ($7–$14/ft,
$1,500–$4,000). Four pages pose a cost question and answer it with factors only:

| page | the question it poses | dollar figures |
| --- | --- | --- |
| `guides/gutter-cleaning-cost-york-pa.html` | "Typical price ranges" *(promises figures explicitly)* | **0** |
| `guides/seamless-vs-sectional-gutters.html` | "How much do seamless gutters cost?" | **0** |
| `areas/seamless-gutters-dover-pa.html` | "How much does gutter installation cost in Dover?" | **0** |
| `areas/seamless-gutters-hanover-pa.html` | "How much does gutter installation in Hanover cost?" | **0** |

The two area pages are the clearest proof that this is structural rather than
sloppy: `AREA_SYSTEM` (`growth/techniques.py:256-257`) tells the writer **"never
quote a number"**, and the FAQ generator is instructed to write "a real
follow-up question a York County homeowner would ask next". **The engine
reliably generates the cost question and is then forbidden from answering it.**
Meanwhile `MONEY_SYSTEM` (`growth/techniques.py:379-390`) tells a different
writer that refusing to discuss price is *why* lead-gen sites outrank
contractors. Yesterday called this a contradiction; today it has a count.

**8 — A hypothesis of my own that I tested and killed.** I wondered whether the
Philadelphia impression flood was self-inflicted — whether some page mentions
those towns and Google is matching on it. `strengthen_pages` did write off-area
places onto the site once, which is why the geo guard exists (commits `851aa5d`,
`463ab8a`). **I grepped all 41 pages plus `index.html` for 22 of the flood
...
Merion, Willow Grove, Blue Bell, Hatboro, Spring City, Radnor, Lower Gwynedd,
East Norriton — plus "Montgomery" and "Philadelphia". Zero hits.** The geo guard
is holding and the flood is not coming from our content. Negative result,
recorded so nobody re-tests it.

### What I researched today

**1. The rank-tracker explanation for the flood is weaker than this journal has
been assuming, and I want that on the record against my own prior entries.**
Google **deprecated the `&num=100` URL parameter in September 2025**,
specifically to stop rank-tracking tools generating impressions at deep
positions — the exact mechanism this journal has been invoking since 08-14 to
explain 12,000 out-of-county impressions at positions 10–35. If that loophole
has been closed for eleven months, "rank tracker" needs a new mechanism to be
true. I am not replacing the theory, because **the single strongest piece of
evidence survives it intact: `gutter installer`, position 1.0, 470 impressions,
0 clicks, over a 28-day window.** Position one with a zero click-through across
470 impressions is not something a human sample does. Whatever is generating
these, it is not homeowners. But I no longer think this journal has correctly
identified *what*.
- https://www.incremys.com/en/resources/blog/rank-tracking
- https://yositeup.com/blog/google-search-console-data-guide-2026

**2. The Business Profile is the substrate for AI answers, not just the map
pack — which re-prices the recommendation I have made eleven times.** 2026
sources: **78% of local-services brands are invisible to AI answer engines**;
only ~1.2% of local businesses get recommended. What matters here is the
mechanism — ChatGPT pulls Business Profile information directly into local
answers, Gemini reads GBP as authoritative, and Google's AI Overviews use GBP as
the spine of the output. Also useful: **ChatGPT and Gemini cite the same domain
only 8% of the time and recommend the same business 4.2% of the time**, and they
source differently — Gemini leans on business websites, ChatGPT on Reddit and
directories.

This matters because `ai_visitors` has been 0 for twenty-six days and this
journal has treated GBP and AI-visibility as two separate projects. They are one
project. **The profile work already at the top of this list is also the AI work**
— which is a better argument for it than any I have made in eleven attempts.
- https://www.marketingcode.com/ai-search-78-percent-invisible-may-core-update-geo-playbook-may-2026/
- https://martech.org/business-websites-dominate-geminis-local-ai-citations/
- https://www.cheers.tech/geo-academy/ai-search-engine-source-differences

**3. A number from our own code that reframes the whole site-versus-profile
question, and that I should have found weeks ago.** `growth/scout.py:159`
carries the business context the scout is given: *"Google Business Profile is
live: 4.2 stars, 13 reviews, roughly 300 profile views a month."*

**Three hundred profile views a month.** The site gets two to three visitors a
day — call it 75 a month. **The Business Profile is already reaching roughly
four times as many people as the entire website**, including all 41 pages this
engine has generated. Every on-site technique in the ledger is optimising the
smaller channel. I am flagging the figure's provenance honestly: it is a
hardcoded string in a prompt, of unknown vintage and unknown accuracy, not a
measurement. But it is the only estimate of profile reach anywhere in this
system, and if it is even roughly right it inverts the priority order this
journal has been using.

**4. Phone calls, not forms, are how this business gets hired.** **78% of home
service leads originate from phone calls**, ~22% from forms and chat;
click-to-call on mobile converts at 8–12%, roughly 3× form submission; median
contractor sites convert 2–4% of visitors, while sites that clearly name their
specific services reach 12–16%. **I am recording this as a rejection, not a
...
- https://pipelineon.com/blog/get-more-calls-contractor-website/

**Rejected today, with what I checked.**
- **Anything about making the phone number more prominent.** I audited all 41
  pages plus `index.html`: **every single one has a `tel:` link, five of them on
  a typical page, eight on the homepage.** The sticky mobile call bar is shipped
  (`growth/templates.py:160`, `styles.css:229-268`). Research item 4 describes a
  problem this site does not have.
- **Adding FAQPage structured data.** `growth/techniques.py:195` emits it, and
  `:1670-1717` deliberately avoids adding a second block where one exists.
- **`services/half-round-gutters.html` looked like a generation defect** — 42KB
  and 1,585 lines against ~19KB for its neighbours. I checked for duplicated
  paragraphs and oversized JSON-LD and found neither: 9 sections, no repeats,
  2.3KB of schema. It is simply a long page. Not a bug.
- **Proposing Foursquare / BuildZoom / Apple Maps.** T048, T053, T029, already
  candidates, re-ranked yesterday.
- **Any tactic aimed at impressions.** 26,613 impressions bought 18 clicks.

### Recommendations

**Nothing in this commit is live.** The site runs from
`/var/www/nemo-seamless-gutter`, which is not a git checkout, and
`growth/publish_state.sh` copies droplet → repo only. Every item below needs a
deploy or a human before it changes anything.

1. **Divine: send me the `snapshot: FAILED` line from
   `/var/log/nemo-growth.log`, and deploy `growth/`.** *(Minutes.)* The log line
   names the exception; I cannot see it and cannot fix the cause without it.
   Deploying also ships the watchdog check below, so the next occurrence
   announces itself. **New argument for the deploy, day 14: I ran the suite and
   it is green — 218 tests.** It also carries `coverage_pct` → 39.5%, the price
   queue fix, and the nine `crawl_*` counters that answer T046.
   *How you would know:* a snapshot dated the same day as its build appears in
   tomorrow's publish commit. *Checked:* `growth_daily.py:220-228` for the
   swallowed exception; `growth_daily.py:237` for the watchdog's blind spot.

2. **Divine: top up the Anthropic credit — and cap the denominator in the same
   sitting.** *(Minutes, small money.)* Unchanged from yesterday and still
   right: restoring credit alone restarts the scout, which grows the tracked
   universe faster than the engine can cover it, so `share_pct` falls while
   nothing gets worse. Options unchanged: **(a)** cap `tracked_queries`;
   **(b)** promote `top3 / ranked_known` — **2 of 25** — to the headline;
   **(c)** rate-limit the scout to the build rate. *Caveat I have to state:* I
   cannot confirm the credit is still empty, because today's build log was never
   published. It was empty on 08-19 and 08-20.

3. **Eric: twenty minutes on the Business Profile — primary category, then
   per-town service areas, then itemised Services.** *(Free. T016/T051/T022,
   day 26. Eleventh appearance.)* **The new reason to do it is research items 2
   and 3, and it is a better reason than any I have given before.** The profile
   is reaching ~300 people a month against the website's ~75, *and* it is the
   data source ChatGPT and Gemini read when they answer local trade questions —
   so this single unglamorous task is simultaneously the map-pack lever, the
   biggest-channel lever and the entire AI-visibility strategy, which
   `ai_visitors: 0` for twenty-six days says is currently at zero. List each town
   as its own service-area entry rather than one polygon. Skip the Q&A half of
   T016; the feature is gone. *How you would know:* the `york` bucket moves off
   33/5/0, or `local_visitors` lifts off ~0, within a month. *Checked:* T016,
   T051, T022 all `candidate`/`activated: null`; and `grep` over `growth/*.py`
   confirms **nothing in the engine touches the Business Profile** — every
   active technique writes files into the site. This one cannot be automated.

4. **Eric: one job photo and one short update to the profile per week, starting
   this week.** *(Free. T008 and the photo half of T022.)* The seasonal item:
   the window to be ranking for leaf-fall closes around mid-September. Actual
   jobs, actual town names, no stock images, no incentives. *Checked:* both
   `candidate`/`activated: null`.

5. **Eric: reply to all 13 reviews in one sitting, then ask every finished job
   for one.** *(Free. T033, T007.)* Response rate is the half of the review
   signal that costs nothing. No incentives, no gating, no filtering.

6. **Eric: confirm the PA HIC registration number and put it on the site, the
   profile and the estimate form.** *(Free if registered. Day 7.)* PA law
   requires the registration number on advertising distributed in Pennsylvania.
   Legal exposure, not a growth idea, and exempt from any cadence change.

7. **Eric decides the price policy; the engine implements it. Now with a count:
   four pages, not one.** *(See item 7 above for the table.)* The decision is
   whether NEMO publishes ballpark ranges at all. *If yes,* make the
   no-figures rule intent-aware so cost pages follow `MONEY_SYSTEM`, and
   regenerate the four. *If no,* one edit is still required regardless of policy:
   **delete the "Typical price ranges" heading and its "With that caveat:"
   sentence from `guides/gutter-cleaning-cost-york-pa.html`** — a section that
   announces figures and withholds them is worse than no section, under either
   answer. **I did not make that edit** — see below. *Checked:*
   `growth/techniques.py:256-257`, `:379-390`, `:1580-1583`.

8. **Eric: reject fifty-nine of the sixty-two candidates.** *(Day 14.
   `does_not_work` empty on day twenty-six.)* Shortlist unchanged: **T048, T053,
   T029**. Start rejections with **T011** (Local Services Ads) — mid-migration
   into Google Ads, ~$53/lead, dashboard scheduled for retirement for NEMO's
   business class.

### What I changed in this repo today

**One fix, with tests.** `growth_daily.py`: a new `_snapshot_problems()` helper,
called from `cmd_watchdog`, that fires when the build completed today but
`growth/snapshot.json` still carries an older date — exactly this morning's
shape. It stays silent when the engine did not run at all, because the existing
build-staleness check owns that case and two alarms for one cause teaches people
to ignore both. `growth/test_watchdog.py`: six tests, one of which reproduces
today's failure by name. **Suite is green: 218 tests.**

I made this one and not the copy edits because it is the failure that hid every
other failure, it is engine plumbing rather than a business decision, and it is
covered by tests. **I again did not touch the price copy**, for the reason
yesterday's entry gave and I still agree with: whether this business quotes
ballpark numbers is Eric's call, not mine to make at 11am while nobody is
reading. Recommendation 7 isolates the one edit that is correct under either
answer, so it is ready the moment he picks one.
...
**Corrections to my standing instructions.** The prompt still describes the
07-28 baseline (77 rows, 17 matched, 3 clicks, 429 impressions, avg position
12.4; county bucket 2 of 50) and the 08-01 usage cap as current. Last actual
reading: **964 / 25 / 18 / 26,613 / 25.1**, county bucket **2 of 103**, and the
08-01 cap expired three weeks ago — the blocker since has been an **empty credit
balance**, a different error with a different fix. The prompt's advice to
suspect the measurement if `visitors` sits at zero for a week is stale: since
the 08-14 beacon cutover the honest count is ~2–3/day. **And one to add: the
prompt tells me to read `snapshot.json` as today's state. It is not always
today's. Check `snapshot.date` against the date of the publish commit before
believing a single number in it** — I would have reviewed stale data as fresh
this morning if I had not looked at `git log` first.

### Reasoning and uncertainties

Day twenty-six, and the most useful thing this review did was notice a file that
was not there.

**What I would defend hardest today.** I did not fire either of the two
tripwires that came due, and I did not quietly drop them either. The credit
tripwire is genuinely unresolvable — the evidence was never written — and
escalating on it would have meant asserting a third failure day I cannot see.
The cadence tripwire I revised on its merits and said exactly why. Yesterday I
weakened a tripwire because my own evidence contradicted it; today I declined to
fire two because the evidence is absent. Those are the same discipline.

**What I got wrong before today.** Eleven entries recommended the Business
Profile without anyone noticing that `growth/scout.py:159` contains an estimate
that the profile out-reaches the entire website four to one. It has been sitting
in a prompt string in this repo the whole time. Two days running, reading the
source has beaten reasoning from the metrics — and the general lesson is
sharper than "field names are not self-documenting": **the prompts this engine
feeds its own writers contain business facts that appear nowhere in the
snapshot.**

**Where I am least confident. Four things.**

*First, I cannot diagnose today's snapshot failure, only detect it.* My leading
guess is the `growth_daily.py` + `indexstatus.py` change Divine committed on
08-20 (`93c78c2`) — a new module landing the day before the snapshot stops
writing is suspicious timing. But `snapshot.py` does not import `indexstatus`,
`indexstatus.summary()` uses `.get()` throughout and cannot raise, and if the
package on the droplet were mismatched the run would have died at import rather
than reaching `rebuild_sitemap`. **So the timing is suggestive and the mechanism
does not work, and I would rather say that than offer Divine a tidy theory that
sends him down the wrong path.** The log line settles it in seconds.

*Second, the 300-profile-views figure is doing a lot of work for a hardcoded
string.* If it is stale or invented, recommendation 3's headline argument
weakens — though the map-pack and AI-substrate reasons stand on their own. It is
checkable in the GBP dashboard in about fifteen seconds, and worth checking
before anyone leans on it as hard as I just did.

*Third, on the flood I have now weakened my own explanation without supplying a
better one.* The `num=100` deprecation is a real, dated, mechanical fact and it
undercuts the rank-tracker theory this journal has repeated for a week. The
position-1-zero-clicks row still says these are not humans. I would rather hold
"not human, mechanism unknown" than keep repeating an explanation I have found a
hole in.

*Fourth, and unchanged: twenty-six days, two top-3 queries, zero in every named
town, three bookings all time, and no call to the phone assistant since 07-30.*
The recommendations are not getting better; they are getting older. That is why
the cadence is changing.

**What would change my mind, dated.**
1. **The next fresh snapshot:** if it is dated the same day as its publish
   commit, the failure was transient. If a second morning publishes without one,
   the feed is broken structurally and everything above this line is guesswork.
2. **On the credit:** the first published build log settles whether 08-21 was a
   third consecutive failure or a self-heal.
3. **On deploy:** `coverage_pct` **39.5%**, `price.covered` **8**, and `crawl_*`
   present. If `crawl_gptbot` is 0 while `crawl_googlebot` is not, AI
   invisibility is an access problem and T046 becomes urgent. If both are
   non-zero, the crawlers come and do not cite us, and research item 2's reading
   — that the answer runs through the Business Profile — is the right one.
4. **2026-09-08:** the flood-decay test. I still predict the 40 rows are there.
5. **2026-09-15:** the seasonal window closes. If the profile is untouched by
   then, leaf season 2026 is decided, and the honest thing to write is that this
   channel never got its chance — not that it did not work.
6. **On the profile:** if Eric does 3, 4 and 5 and the `york` bucket still reads
   33/5/0 with no calls a month later, the map pack is not the lever either, and
   the real question becomes whether a one-truck operator can win this county on
   search at all.
...
## 2026-08-29 — engine run

Goal: **1.0%** top-3 share of 195 tracked queries (target 50%).
2026-08-28: 3 visitors (0 organic, 0 maps) · 0 bookings, 0 phone leads.

**Built:**
- `adopt_queries` — ok: no new in-area searches worth tracking
- `improve_ctr` — ok: no page is due a snippet rewrite
- `geo_answer_first_content_pass` — ok: every ranking page already opens with a direct answer
- `strengthen_pages` — ok: added "Soffit and Fascia Repair in Spring Grove, PA" to /areas/seamless-gutters-spring-grove-pa.html for 'soffit and fascia repair spring grove pa'
- `service_pages` — ok: every queued service already has a page
- `area_pages` — ok: every queued town already has a page
- `money_pages` — ok: no query needs its own page — the remaining gaps all belong on pages that exist, which strengthen_pages handles
- `internal_links` — ok: refreshed nearby-links on 0 page(s)
...
## 2026-08-30 — review agent

### The deploy did not land. Yesterday's entry led with a claim that today's data refutes.

Yesterday I wrote *"The blackout is over. The deploy landed yesterday afternoon."*
The run resumed. The **code did not ship**, and I can prove it three ways from
today's `snapshot.json` without leaving this repo:

| what the repo's `snapshot.py` emits unconditionally | in today's published snapshot? |
| --- | --- |
| `code_version` (`snapshot.py:231`) — the fingerprint block whose entire purpose is answering this question | **absent** |
| `keywords.ranked` (`snapshot.py:200`) — the goal metric, one row per query | **absent** |
| `gsc.pages` (`snapshot.py:208`) | **absent** |
| `log_visitors` / `log_pageviews` / `crawl_*` in `SERIES` (`snapshot.py:47-51`) | **absent** — published `traffic` has 14 keys, none of them these |

And `review.py`: the six unfounded verdicts I documented yesterday were **restamped
this morning**, `"decided": "2026-08-30"`, with the same strings —
T001 `works: true`, why *"7 owned visitors in 34d (median 0.0/day and flat)"*.
`scoreboard.works` is still the same seven. Yesterday's change-my-mind test #2 was
*"`scoreboard.works` dropping from 7 to 2 confirms the fix ran; still 7 means it did
not deploy."* **It is still 7.**

What actually happened on 08-28 is legible in the commit history: `e26c136` (08-26)
fixed a crash in **`growth_daily.py`**, a single root-level file, and touched no
other code. Deploying that one file restarts the run without updating anything
under `growth/`. That is consistent with every observation: the cron completes, the
snapshot publishes, and the engine producing it is a **file-by-file patchwork of
different vintages**.

Best current reading of what is live, and how confident I am:

| file | evidence | vintage |
| --- | --- | --- |
| `growth_daily.py` | the run works again after `e26c136` | ~08-26, **high** |
| `techniques.py` | `by_intent.price.covered` rose 3 → 8; with a strict intent sort `price` is unreachable while eligible `hire` rows remain, so the `313b2ff` round-robin is running | ≥08-16, **medium** |
| `snapshot.py` | four independent keys missing, above | **older than this repo's first commit**, high |
| `review.py` | verdicts restamped 08-30 with pre-fix text | <08-29, **high** |
| `techniques.py` `_strlist` (08-27) | **no evidence either way** | unknown |

That last row matters and I want to retract something. Yesterday I wrote that
`_strlist` "is in the tree the droplet is running." **That was an inference from a
deploy that did not happen, and I withdraw it.** The guard against the
letter-shredding bug may or may not be live. `strengthen_pages` wrote to a live page
again this morning.

I have stopped recommending "deploy `growth/`" as prose. It has been recommended for
twenty-five days. Today I shipped a script instead — see *What I changed*.

### Where the numbers stand

**The goal metric did not move. Day thirty-four.**
...
| `top10` | 8 | 9 |
| `ranked_known` | 25 | 25 |
| `coverage_pct` | 43.1% | 43.6% |

Per town, `total / covered / top3` — one cell moved, dallastown covered 4 → 5, which
is this morning's section:
...
**Zero top-3 positions in every named town, day thirty-four.** Both top-3 queries are
in the county bucket, where they have been since before the measurement blackout.

**Search Console**, 28-day window ending 3 days back: rows 968 → **964**, matched 24 →
**24**, clicks 18 → **17**, impressions 26,724 → **26,623**, avg position 25.2 → **25.1**.
Per my own standing correction I am not reading `avg_position` or site CTR as health;
see the next section for why that is now doubly true.

**Traffic**: 08-29 was **1 visitor**, 0 organic, 0 local, 0 AI. The last eight days are
1, 2, 2, 2, 3, 1 — median 2. `ai_visitors` **0 for all 34 measured days**.
`local_visitors` 0 since 08-17.

**Leads: 3 bookings all time, 0 phone leads ever.** Last booking **2026-08-11**,
nineteen days ago. `call_taps` has fired **once**, on 08-12. This is the number that
matters and it has not moved in nineteen days.

### Finding — the Philadelphia flood is a fixed historical burst that is about to drain out of the window, and I had this wrong yesterday

Yesterday I concluded the flood was **"steady-state, not a decaying transient"** and
predicted impressions hold at 24,000–29,000 through 09-08, with "below 20,000 refutes
me". I reached that from site-wide totals moving only +0.4% across a 9-day window
shift. **That was the wrong unit of analysis.** Today I compared the 40 rows
themselves across every snapshot in the repo:

| snapshots compared | window slide | rows byte-identical |
| --- | --- | --- |
| 08-28 → 08-30 | 2 days | 39 / 40 |
| 08-20 → 08-30 | 10 days | **38 / 40** |
| 08-09 → 08-30 | 21 days | 14 / 39 — and the changes are **+2 to +14 impressions** |

The window on 08-09 was 07-10 → 08-06; on 08-30 it is 07-31 → 08-27. **Twenty of
twenty-eight days have been replaced and these queries did not notice.** A query
accruing impressions steadily cannot behave like that. A query whose impressions are
a fixed block sitting inside the intersection of both windows — **07-31 to 08-06** —
behaves exactly like that.

Two independent things corroborate the date. `_select_discoveries`' own docstring
(`gsc.py:307`), written at the time by an earlier run: *"On 2026-08-04 Google began
showing the site across the Philadelphia suburbs."* And one row is **already
draining**: `gutter installer` went 471 (08-13) → 470 (08-20) → **362** (08-30),
−108 in ten days. That is what a burst leaving a rolling window looks like, visible
on the one row whose burst was earliest.

> **Prediction, dated, replacing yesterday's.** The 28-day window on **2026-09-06**
> starts **08-07** — the first day that excludes a 08-04→08-06 burst entirely. So
> between roughly **2026-09-01 and 2026-09-08**, site impressions **fall from ~26,600
> to somewhere near 15,000**, and `avg_position` **improves sharply**, plausibly from
> 25 into the low-to-mid teens, as forty rows sitting at position 10–35 leave the
> average. **Neither is a ranking loss and neither is a ranking win.** If impressions
> instead hold above 24,000 through 09-08, I am wrong and yesterday's reading was
> right.

**This is the most important thing in today's entry for anyone reading the numbers next
week.** Impressions falling 45% in a single week looks like a catastrophe and it is
arithmetic. `avg_position` improving 25 → 14 looks like the engine finally worked and
it is the same arithmetic. Write both down now, before they happen.

### Finding — the engine has very nearly run out of work

Today's build: **`new: 0, changed: 1`**. Eight of eleven steps reported `noop: true`:

> `adopt_queries` "no new in-area searches worth tracking" · `improve_ctr` "no page is
> due a snippet rewrite" · `geo_answer_first_content_pass` "every ranking page already
> opens with a direct answer" · `service_pages` "every queued service already has a
> page" · `area_pages` "every queued town already has a page" · `money_pages` "no query
> needs its own page" · `internal_links` "refreshed nearby-links on 0 page(s)" ·
> `local_schema` "already current"

The single unit of work the engine now does per day is one `strengthen_pages` section,
by construction: the loop at `techniques.py:1036-1108` **returns on its first
success**. There are **110 uncovered tracked queries**. At one a day that queue clears
around **2026-12-18** — three weeks after leaf-fall season ends.

And the queue is not ordered by demand. `strengthen_pages` buckets by intent and
round-robins (`techniques.py:1024-1029`); within an intent it preserves whatever order
`keywords.load()` returns. Today's one action went to *"emergency gutter repair near me
dallastown pa"* — Dallastown, population ~4,000 — while **"gutter installers york pa",
"seamless gutter installation york pa", "gutter company york pa" and "gutter
contractors york pa" all sit uncovered** in the same list. The keyword records carry
`impressions`, `clicks` and `position` from Search Console (`keywords.py:347-349`) and
**nothing in the queue reads them**.

I did not make that change, and the reason is a number I cannot see from here: only
**24 of 195** tracked queries matched a GSC row this morning, so most uncovered queries
probably carry `impressions: null` and an impressions sort could be nearly inert.
`keywords.json` is gitignored and I cannot count them. That check is recommendation 5.

### Finding — the `gsc_clicks` unit error is not contained, and it is now writing the ledger's hypotheses

Yesterday I judged the 28× unit error in `gsc_clicks` harmless once `earned()` refused
the no-baseline branch. **It is not harmless.** This morning's scout proposed T076 and
its stated hypothesis begins:

> *"300 GBP views and **~18 GSC clicks a day** produce zero calls."*

The true figure is **17 clicks per 28 days ≈ 0.6/day**. The scout is reasoning about a
business with thirty times the traffic it has, and that reasoning is now permanently
recorded in the ledger as T076's justification. T076 may still be a good idea — putting
Eric's 13 real reviews on the town pages is cheap and sound — but it was argued from a
number that is wrong by a factor of thirty, and every future scout run will be too.
Fixing the units at the source is no longer optional; see recommendation 6.

### Did previous changes work?

**1 — Deploy `review.py`/`ledger.py` (yesterday's rec 4). NOT ACTIONED.** Six verdicts
restamped `2026-08-30`, `scoreboard.works` still 7. Resolves yesterday's test #2 in the
negative. The commit `51cf063` landed 08-29 11:14 UTC; the 06:08 cron the next morning
did not pick it up, because nothing copies repo → droplet.

**2 — Repair the live half-round page (rec 3, day fourteen). NOT ACTIONED.**
`services/half-round-gutters.html` is still **1,600 lines with 1,254 single-letter
`<p>` elements**, unchanged in today's publish. Resolves yesterday's test #1 in the
negative. I re-checked every generated page: it is still the only one affected.
**Fourteen days of a live customer-facing page rendering one letter per paragraph.**

**3 — "The deploy landed" (yesterday's lead). WRONG, corrected above.** The run
resumed; `growth/` did not ship.

**4 — Scout parse failure (yesterday's rec 8: "read the file, don't patch the
parser"). RESOLVED ON ITS OWN.** `last_scout` today is `ok: true` with T076 added. One
bad reply, no code change, no recurrence. Recording this because "don't harden against
a single failure" was the right call and it is worth having a case where restraint won.

**5 — Coverage work by `strengthen_pages`, nine mornings. TOO EARLY TO TELL, on
schedule.** Coverage 32.3% → 43.6% since 08-20; `top3` **2 → 2**. Pages published in
the last week are 0–6 days old and local commercial terms take weeks, so the absence of
movement refutes nothing yet. The dated test stands: **2026-09-20**.

**6 — Anthropic credit (08-24). Holding, day six.** Content generation ran again this
morning.

**7 — Eric's list (GBP, reviews, HIC), day thirty-four. UNACTIONED.** See
recommendations 1 and 2.

**8 — `internal_links` to guides (rec 7, from 08-24). NOT ACTIONED.** Re-verified in
code: `techniques.py:567,574` reads `_all_area_pages(ctx)` only. It also reported
"refreshed nearby-links on 0 page(s)" this morning, so it is doing nothing at all now.

**9 — Output sanity check on generated pages (rec 6, from 08-27). NOT ACTIONED.**
Re-verified: `grep -nE "median|paragraph_count|sanity|_validate" growth/techniques.py`
returns nothing.

### What I researched today

- **Whitespark 2026 factors, re-checked for anything new since yesterday.** The number
  that matters is sharper than I had it: **review velocity moved from ~#93 to ~#11**,
  the largest year-over-year jump recorded, and the practitioner reporting adds a
  threshold — *businesses that stop receiving new reviews for roughly three weeks see
  measurable ranking declines*. NEMO's 13 reviews have no evidence of recency at all.
  This is not "get more reviews", it is **"get one most weeks, starting now"**.
  https://minyona.com/blog/google-business-profile-contractors ·
  https://pipelineon.com/blog/google-reviews-strategy-for-contractors/
- **Proximity tightened in 2026.** Google reduced the radius within which non-proximate
  businesses rank for high-intent local queries; service-area businesses outside the
  immediate area saw pack drops. Two consequences here. It **confirms the flood is
  worthless** — NEMO cannot enter a Glenside map pack at ninety miles. And it says the
  realistic ceiling on Hanover or Shrewsbury from a York-centric base is lower than the
  50% goal assumes, which is worth Eric knowing before he judges the year on it.
  https://www.digitalapplied.com/blog/local-seo-march-2026-core-update-gbp-optimization-guide
- **Listing each town as its own service-area entry beats one broad polygon** for those
  towns' queries — sharpens standing rec 2(c), which is already in the list.
  https://www.merchynt.com/post/google-business-ranking-guide-2026
- **GBP Q&A is gone**, replaced by Gemini-powered "Ask Maps" in early 2026 — profile
  field completeness now feeds the answer instead. Confirms yesterday's instruction to
  skip T016's Q&A half; the rest of T016 gets *more* important, not less.
  https://www.scatterbranchmarketing.com/blog/is-your-google-business-profile-ready-for-2026-what-changed-and-what-to-do-now
- **Rejected: the standard "high impressions, low clicks" remedy** (rewrite titles and
  descriptions). Same rejection as yesterday and today's finding hardens it — the
  impressions are a historical burst in someone else's county that is about to
  disappear on its own. Rewriting snippets to chase them would optimise for people who
  cannot hire NEMO, and would then appear to "work" in early September for reasons
  that have nothing to do with the rewrite.
  https://www.localseotactics.com/how-to-fix-high-impression-and-low-click-pages-in-google-search-console/
- **Rejected, unchanged:** Local Services Ads and paid anything (costs money, T011/T040
  already candidates, not mine to spend); call tracking / DNI (at 2 visitors a day it
  attributes nothing and carries NAP risk); AI-answer-engine work (`ai_visitors` 0 for
  34 days; AI answers cite whoever wins the map pack, so it is not a separate lever).

### Recommendations

**Nothing in this commit is live.** The site and engine run from
`/var/www/nemo-seamless-gutter`, which is not a git checkout; `publish_state.sh` copies
droplet → repo only. `areas/`, `guides/`, `services/`, `index.html` and `sitemap.xml`
here are a **read-only mirror** — a page cannot be fixed in this repo. Note also that
the developer's report went weekly on 08-27 and the watchdog no longer mails: today is
**Sunday**, so the next developer email is **Friday 2026-09-04**.

1. **Eric — ask every completed customer for a Google review, this week and every
   week.** *(Minutes per job. Free. Day thirty-four.)* Review **recency** is ~#11 among
   all local ranking factors, up from ~#93; review signals are ~20% of local-pack
   weight; a three-week gap is measurable. No incentive and no gating — both violate
   Google's terms and a suspension would cost more than every tactic in this ledger
   could earn. **Sixteen days to the mid-September leaf-season window.** Needs no
   deploy, no code, and no working engine. *Checked:* nothing in `growth/*.py` touches
   reviews or the Business Profile — every active technique writes files into the site.
   Cannot be automated.
2. **Eric — twenty minutes on the Business Profile.** *(Free. T016 / T051 / T022, day
   34.)* (a) primary category exact; (b) trim secondary categories to 2–4; (c) each town
   its **own** service-area entry, up to the 20-area cap, only towns Eric will drive to
   — today's research says separate entries beat one broad polygon; (d) itemised
   Services. Skip the Q&A half of T016, the feature no longer exists. *Checked:* T016,
   T051, T022 are all still `candidate` / `activated: null` in today's snapshot.
3. **Divine — run the half-round repair. Two minutes. Day fourteen.**
   ```
   git -C /root/nemo-repo fetch origin main && git -C /root/nemo-repo reset --hard origin/main
   python3 /root/nemo-repo/deploy/repair_letter_paragraphs.py --root /var/www/nemo-seamless-gutter
   python3 /root/nemo-repo/deploy/repair_letter_paragraphs.py --root /var/www/nemo-seamless-gutter --apply
   ```
   Dry-run first; backs up; idempotent; imports nothing from `growth/`. *How you would
   know:* tomorrow's publish shows that file shrinking by ~1,260 lines. *Checked:* still
   1,600 lines / 1,254 single-letter `<p>` in today's mirror.
4. **Divine — run `deploy/deploy_growth.sh` (new today). One minute, read-only.**
   ```
   git -C /root/nemo-repo fetch origin main && git -C /root/nemo-repo reset --hard origin/main
   ...
   ```
   The report alone settles a question this journal has been guessing at for three
   weeks: it hashes every engine `.py` on both sides and prints ok / STALE / MISSING per
   file. **Please paste that table into a commit message even if you do not run
   `--apply`** — it replaces every inference in the vintage table above with fact.
   `--apply` copies only `.py` files, backs up what it overwrites, and never touches
   `techniques.json`, `keywords.json`, `results.jsonl` or `state.json`. *How you would
   know it worked:* tomorrow's `snapshot.json` gains a top-level `code_version` block,
   and `scoreboard.works` drops from 7 to 2. *Checked:* no such script existed —
   `grep -rln rsync --include='*.sh'` found only `publish_state.sh`, which is one-way
   droplet → repo by design.
5. **Divine — one command, and it answers whether recommendation 7 is worth building:**
   ```
   ...
   ```
   If most uncovered queries carry impressions, sorting the queue by demand is worth
   doing. If almost none do — likely, since only 24 of 195 matched this morning — then
   the right ordering key is a human judgement about which towns Eric wants, not a
   metric, and rec 7 should not be built. *Checked:* `keywords.json` is gitignored and
   absent from this repo; I genuinely cannot answer this from here.
6. **Engine — fix the `gsc_clicks` units at the source.** *(Small.)* `gsc.py:283`
   records the **28-day window total** once per day; `review.py` and now the scout both
   read it as a daily rate, a 28× overstatement that is currently writing false premises
   into the technique ledger (see the T076 finding). Record it under a new name with the
   correct units and leave the old series in place — renaming in situ orphans the
   existing `results.jsonl` on the droplet, which is why I did not do it myself.
   *Checked:* `gsc.py:283`, and T076's hypothesis text in today's snapshot.
7. **Engine — order the `strengthen_pages` queue by demand, if rec 5 says the data
   exists.** *(~4 lines: sort each intent bucket by `impressions` descending, nulls
   last.)* One action a day and 110 queries left means the order decides what gets
   written before leaf season, and today it is arbitrary. *Checked:* `techniques.py:
   1005-1029` reads `intent` only; `keywords.py:347-349` proves the impressions field is
   populated for matched queries. **Not implemented.**
8. **Engine — output sanity check on every generated page.** *(~20 lines. Carried from
   08-27, still unbuilt, and now more urgent: I can no longer claim the `_strlist` guard
   is deployed.)* `<p>` count, median paragraph length, file-size delta; median under
   ~20 chars or >3× single-pass growth marks the technique `ok: false`. Would have
   caught 08-16 on 08-16 instead of on day fourteen. *Checked today:*
   `grep -nE "median|paragraph_count|sanity|_validate" growth/techniques.py` returns
   nothing.
9. **Engine — extend `internal_links` to guides.** *(~30 lines. Carried from 08-24.)*
   Now doing literally nothing — "refreshed nearby-links on 0 page(s)". *Checked:*
   `techniques.py:567,574`, area pages only.
10. **Eric and Divine — decide what `MIN_RECENT_MEDIAN` should be.** *(Carried from
    yesterday, unanswered.)* As written the retire branch is unreachable and nothing can
    ever be marked `does_not_work`. My suggestion stands: test on
    `total < MIN_TOTAL_VISITORS` alone, after ~60 days. A human call, not mine.

### What I changed in this repo today

228 tests pass (`python3 -m unittest discover -s growth -t .`). I touched no runtime
state, activated or retired nothing, and edited no keyword list, page or engine module.

**`deploy/deploy_growth.sh` — new.** Twenty-five days of recommending "deploy `growth/`"
in prose has produced one partial deploy of one root-level file. The repair script that
shipped on 08-28 is the format that got read, so this is the same format.

Its **report mode is the more valuable half** and is read-only: it hashes every engine
`.py` in `/root/nemo-repo` against `/var/www/nemo-seamless-gutter` and prints a per-file
ok / STALE / MISSING table. That is the `code_version` diagnostic, available today,
without deploying the file that would otherwise have to ship first to provide it — the
circularity that has kept this question open since 08-09. `--apply` backs up to a
timestamped directory, copies **only** `.py` files (never the runtime `.json` /
`.jsonl` state that lives beside them), prints the undo command, and is idempotent.

Tested against a simulated docroot: report mode correctly identified a hand-edited file
as STALE and a deleted one as MISSING; `--apply` copied exactly those two, backed up the
one that existed, left a planted `techniques.json` untouched, and a re-run reported
0 of 31 differing.

I deliberately did **not** implement rec 7 (I cannot see `keywords.json` to know whether
the data supports it), rec 6 (renaming a live metric orphans `results.jsonl` on a
machine I cannot reach), or rec 10 (a live auto-retirement policy is not mine to set).

**Corrections to my standing instructions.** *New today, three:* (a) **the 08-28 event
was not a `growth/` deploy** — the run resumed, the code did not ship, and yesterday's
lead is withdrawn; (b) **yesterday's "`_strlist` is now deployed" is withdrawn** — it
rested on that same false premise, and no evidence either way exists; (c) **the
Philadelphia flood is a fixed burst dated to roughly 2026-08-04, not a steady state** —
yesterday's 09-08 prediction is withdrawn and replaced above, and the coming impression
collapse must not be read as a ranking loss. *Carried:* `does_not_work` being empty is a
code artifact, not a finding; never quote `avg_position` or site CTR as health; the
prompt's 07-28 GSC baseline is a month stale and ~60× low on impressions; the prompt's
08-01 usage-cap story is obsolete; the engine publishes ~5 hours before this review, not
one; `areas/`, `guides/`, `services/`, `index.html`, `sitemap.xml` are a read-only
mirror; read the engine's output, not only its source.

### Reasoning and uncertainties

Day thirty-four. Two of two dated tests resolved, both negative.

**What I would defend hardest.** The deploy finding and the flood finding, because
neither rests on inference about a machine I cannot reach. Four keys that `snapshot.py`
emits unconditionally are absent from the file `snapshot.py` produced; six verdicts
carry today's date and yesterday's buggy text. And the flood: 38 of 40 rows identical
across a ten-day window slide is not something ongoing impressions can do.

**Where I am least confident.** The exact burst dates. The intersection argument bounds
it to 07-31 → 08-06 and `gsc.py`'s own docstring says 08-04, but the small positive
drifts (+2 to +14) say a trickle continues, so the collapse could be partial —
maybe 26,600 → 17,000 rather than → 15,000. The **direction and the week** are what I
am claiming; the magnitude is an estimate. Also: `techniques.py` ≥ 08-16 is an inference
from `price` coverage moving, and recommendation 4's report would replace it with a
hash.

**What I am not claiming.** That any of this makes the phone ring. The engine's daily
output is now one paragraph-sized section on one page, eight of eleven steps do nothing,
and the goal metric has been 2/195 since before the blackout. **Nothing in the automated
half of this system is currently capable of moving the number Eric cares about.**
Recommendations 1 and 2 are, they cost nothing, they need no deploy, and they have not
been done in thirty-four days.

**What I got wrong today, caught before publishing.** My first comparison of the
untracked rows used `git log --grep` to find the 08-09 snapshot, matched no commit,
silently fell back to the working-tree file, and reported "40 of 40 byte-identical" —
the file against itself. I redid it by reading each snapshot's own `date` field. The
corrected numbers are the ones in the table, and they support a *stronger* conclusion
than the artefact did, which is luck rather than judgement. Recording it because
yesterday's entry made a mirror-image error: reasoning about a 45%-out-of-market
aggregate instead of the rows underneath it.

**What would change my mind, dated.**
1. **Tomorrow's publish.** `services/half-round-gutters.html` at ~340 lines says rec 3
   landed; 1,600 says day fifteen.
2. **Tomorrow's snapshot.** A `code_version` block, or `scoreboard.works` at 2, says
   rec 4 landed. Their absence says the engine is still a patchwork.
3. **2026-09-06 → 09-08 — the impression-drain test.** Impressions near 15,000 and
   `avg_position` in the teens confirms the burst reading. Holding above 24,000 refutes
   it and vindicates yesterday's entry.
4. **2026-09-20 — the coverage-versus-rank test.** Coverage 32.3% → 43.6% in ten days
   with `top3` flat at 2. If coverage keeps climbing and `top3` is still 2 on 09-20,
   page-strengthening is not converting into rank and the engine's one remaining daily
   activity needs rethinking.
5. **2026-09-15 — the seasonal window. Sixteen days.** Unchanged and still the whole
   argument: recommendations 1 and 2 are free, need nobody's code, and decide autumn.

## 2026-08-31 — engine run

Goal: **1.0%** top-3 share of 195 tracked queries (target 50%).
2026-08-30: 0 visitors (0 organic, 0 maps) · 0 bookings, 0 phone leads.

**Built:**
- `adopt_queries` — ok: no new in-area searches worth tracking
- `improve_ctr` — ok: no page is due a snippet rewrite
- `geo_answer_first_content_pass` — ok: every ranking page already opens with a direct answer
- `strengthen_pages` — FAILED: 3 candidate(s) failed, last: generation failed for 'half round copper gutters near me': anthropic 400: {"type":"error","error":{"type":"invalid_request_error","message":"Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits."},"request_id":"req_011CeaKeDahBYL3gSYNzgyJm"}
- `service_pages` — ok: every queued service already has a page
- `area_pages` — ok: every queued town already has a page
- `money_pages` — ok: no query needs its own page — the remaining gaps all belong on pages that exist, which strengthen_pages handles
- `internal_links` — ok: refreshed nearby-links on 0 page(s)
