...
// Signed, tokenless management link for the owner's emails (so the link itself is
...
/* ---------------------------------------------------------------------------
 * 1. BUSY READING — pull the owner's calendar feeds (Google secret iCal / iCloud
  ...
  for (const rawUrl of config.icsFeeds) {
    try {
      ...
      const text = await safeFetchText(url, {
        headers: { 'User-Agent': 'NemoSeamlessGutter/1.0 (+https://nemoseamlessgutter.com)' },
      });
...
// so the owner's Apple events block site slots in near real time.
...
/* ---------------------------------------------------------------------------
 * 2. INVITE WRITING — build an ICS (METHOD:REQUEST) and email it to the owner +
...
function buildICS(booking) {
  ...
  const descLines = [
    ...
    '',
    'Booked via nemoseamlessgutter.com',
  ].filter((l) => l !== null);
    ...
    createEvent(
      {
        ...
        attendees,
        productId: 'nemo-seamless-gutter/booking',
      },
...
async function sendBookingEmails(booking) {
  ...
  // To the owner (both addresses): new-booking alert + the invite to add to his calendar.
  await tx.sendMail({
...
// Tell the customer we had to cancel, and point them back to rebooking.
 ...
 *    Calendar so it appears instantly without the owner accepting the email invite.
