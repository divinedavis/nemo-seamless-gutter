// NEMO Seamless Gutter — booking widget
(function () {
    ...
    var svcOpts = state.services
      .map(function (s) {
        return (
          ...
          (s.id === 'consult' ? ' · over the phone' : ' · at your home') + '</span>' +
          '</button>'
        );
      })
      .join('');
      ...
      '<div class="booking">' +
        ...
        '<div class="booking-step"><h3><span class="step-num">3</span>Your details</h3>' +
          '<div class="book-fields">' +
            ...
            '<div class="field" id="bk-addr-field"><label for="bk-address">Job address</label><input id="bk-address" type="text" placeholder="Where is the property?" autocomplete="street-address"></div>' +
            '<div class="field full"><label for="bk-notes">Anything else?</label><textarea id="bk-notes" rows="2" placeholder="One or two stories? Any problem spots? Timeline?"></textarea></div>' +
          '</div>' +
        '</div>' +
  ...
  function submit() {
    ...
    if (state.service !== 'consult' && address.trim().length < 5) { alert('Please enter the job address for your visit.'); return; }
